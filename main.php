...
#
##
if ($_SESSION['user']['pchange'] == '') {
    $_SESSION['user']['pchange'] = $player['pass'];
}
...
if (!preg_match("/{$_SERVER['HTTP_HOST']}/", getenv('HTTP_REFERER')) or $_SESSION['user']['login'] == '' or $_COOKIE['UID'] != $player['pcid'] or $player['block'] != '' or $check['id']) {
    if ($player['block'] != '') {
        echo "<script>parent.location = 'index.php?act=logout';</script>";
    } else {
        echo "<script>parent.location = 'error.php';</script>";
    }
}
...
if (isset($_GET['post_id'])) {
    if ($_GET['post_id'] == 98 or $_GET['post_id'] == 109 or $_GET['post_id'] == 112 or $_GET['post_id'] == 114 or $_GET['post_id'] == 118) {
        include "./gameplay/inc/post_id.php";
    } else if (in_array($_GET['vcode'], $_SESSION['secur'])) {
        include "./gameplay/inc/post_id.php";
    }
}
...
if (!isset($_GET['post_id']) and isset($_POST['post_id'])) {
    if ($_POST['post_id'] == 98 or $_POST['post_id'] == 109 or $_POST['post_id'] == 112) {
        include "./gameplay/inc/post_id.php";
    } else if (in_array($_POST['vcode'], $_SESSION['secur'])) {
        include "./gameplay/inc/post_id.php";
    }
}
if (isset($_POST['fightmagicstart']) and (in_array($_POST['fmc'], $_SESSION['secur']) or in_array($_POST['fmc'], $_SESSION['vcodes']))) {
    include "./gameplay/inc/post_attack.php";
}
#end check post
$player = player();
