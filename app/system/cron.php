...
if ($check3['reklama'] < $time) {
    ...
    mysqli_query($GLOBALS['db_link'], "INSERT INTO `chat` (`time`,`login`,`msg`) VALUES ('" . time() . "','sys', '" . addslashes(">  <font color=000000><b>Пополни свой игровой счёт через мобильный терминал (смс пополнения)(легко и быстро) (МTC,БИЛАЙН,МЕГАФОН) (Россия и Украина) </b></font><BR>'+'');") . "');");
...
while ($row = mysqli_fetch_assoc($Query)) {
    if ($row['bet'] != 'none') {
        mysqli_query($GLOBALS['db_link'], "INSERT INTO `chat` (`time`,`login`,`dlya`,`msg`) VALUES ('" . time() . "','sys','<" . $GLOBALS['DBLink']->query("SELECT `login` FROM `user` WHERE `id`= ? ", array($row['userID']))->fetchColumn(0) . ">','" . addslashes("top.frames['chmain'].add_msg('<font class=chattime>&nbsp;" . date("H:i:s") . "&nbsp;</font> <font color=000000><b><font color=#CC0000>Внимание!</font></b></font>&nbsp;Ваш лот был продан за <b>" . ($row['price'] * 0.95) . "</b> .</font><BR>'+'');") . "');");
        ...
        mysqli_query($GLOBALS['db_link'], "UPDATE `invent` SET `auction`='0',`pl_id`='" . $GLOBALS['DBLink']->query("SELECT `id` FROM `user` WHERE `login`= ? ;", array($row['bet']))->fetchColumn(0) . "' WHERE `id_item`='" . $row['itemID'] . "'");

    } else {
        mysqli_query($GLOBALS['db_link'], "INSERT INTO `chat` (`time`,`login`,`dlya`,`msg`) VALUES ('" . time() . "','sys','<" . $GLOBALS['DBLink']->query("SELECT `login` FROM `user` WHERE `id`= ? ", array($row['userID']))->fetchColumn(0) . ">','" . addslashes("top.frames['chmain'].add_msg('<font class=chattime>&nbsp;" . date("H:i:s") . "&nbsp;</font> <font color=000000><b><font color=#CC0000>Внимание!</font></b></font>&nbsp;Курьер вам доставил вещь с аукциона.</font><BR>'+'');") . "');");
        mysqli_query($GLOBALS['db_link'], "UPDATE `auction_system` SET `status`='finished' WHERE `id`='" . $row['id'] . "'");
...
while ($row = mysqli_fetch_assoc($Query)) {
    $oldAff = test_affect($GLOBALS['DBLink']->query("SELECT `affect` FROM `user` WHERE `id` = ?", array($row['userId']))->fetchColumn(0));
