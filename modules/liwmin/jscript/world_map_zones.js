...
function addZone(x1, y1, x2, y2, zone_id)
{
    ...
    ed1.value = zone;
    ed1.id = 'zone_'+new_id;
    ed1.onclick = function() { selectZone(lid); return false; }
    td2.appendChild(ed1);
...
function drawZones()
{
    ...
    if (zones[c] != '') {
        ...
        div.onclick = function(zone_id) { return function() { if (mode == -1) selectZone(zone_id); } }(c);
...
function createClickZone(x1, y1, x2, y2)
{
    ...
    for (i=y1; i<=y2; i++) {
        ...
        for (j=x1; j<=x2; j++) {
            ...
            dx = j;
            dy = i;
            img.onclick = function(dx, dy) { return function() { mapClick(dx, dy); } }(dx, dy);
