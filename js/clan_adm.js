...
SubmitForm = function(edicode,plid){
	AjaxGet('clan_ajax.php?act=edit&plid='+$('plid').value+'&clan_d='+$('clan_d').value+'&clan_status='+$('clan_status').value+'&access_1='+$('access_1').value+'&access_2='+$('access_2').value+'&access_3='+$('access_3').value+'&access_4='+$('access_4').value);
	FormPopUp('darker');
...
StateReady = function(){
	switch(arr_res[0]){
		...
		s += '</tr>';
		s += '<tr>';
		s += '<td>Статус в клане:</td>';
		s += '<td><select name="clan_status" id="clan_status" style="width:150px;">  <option value="0" '+((str_pr[6]=='0')?'selected="selected"':'')+'>Чмо и отстой (никто)</option>  <option value="8" '+((str_pr[6]=='8')?'selected="selected"':'')+'>Зам.Главы</option>  <option value="9" '+((str_pr[6]=='9')?'selected="selected"':'')+'>Глава Клана</option></select></td>';
		s += '</tr>';
		s += '<td>Казна:</td>';
...
function clan_sostav(pr,nick,lev,signst,sstatus,plnx,plid,fightid)
{
       ...
       if(outcode) slink += '<input type=button class=lbut onClick="javascript: if(confirm(\'Выгнать персонажа?\')) { location=\'?get_id=29&clan_act=2&vcode='+outcode+'&plid='+plid+'\' }" value=" Выгнать ">';
       ...
       if(pr == 1) {priv = '<a href="javascript:parent.say_private(\''+nick+'\')"><img src=http://img.legendbattles.ru/image/chat/private.gif width=11 height=12 border=0 align=absmiddle></a>'; onoff = '<font color="#009933"><b>online</b></font>';}
	   if(pr == 0) {onoff = '<font color=#CC0000><b>offline</b></font>'; plnx = '<font color=gray>Неизвестно</font>';}
       if(salign > 0) all_si = '<img src=http://img.legendbattles.ru/image/signs/'+alar[salign]+'.gif width=15 height=12 border=0> ';
       all_si += '<img src=http://img.legendbattles.ru/image/signs/'+sign+' width=15 height=12 border=0 align=absmiddle>';
       document.write('<tr align=center><td align=left class=nickname bgcolor=white><font class=nickname>'+priv+'&nbsp;'+all_si+' <b>'+nick+'</b>['+lev+']<a href="https://www.legendbattles.ru/ipers.php?'+nick+'" target=_blank><img src=http://img.legendbattles.ru/image/chat/info.gif width=11 height=12 border=0 align=absmiddle></a></td><td align=left class=nickname bgcolor=white><font class=nickname align=center>&nbsp;&nbsp;<b id="user_'+plid+'">'+ssta+'</b></td><td align=center class=nickname bgcolor=white>'+onoff+'</td><td align=left class=nickname bgcolor=white><font class=freetxt align=center>&nbsp;&nbsp;<b>'+plnx+'</b>&nbsp;'+sh_fight(fightid)+'</font></td><td align=center class=nickname bgcolor=white><font class=text>&nbsp;&nbsp;'+slink+'</td></tr>');
}