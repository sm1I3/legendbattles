...
PVUFormat = function(str){
	...
	switch(seen){
        ...
        {
            case 0:
            case 1: r = (arr[2] != arr[3] ? '' : '<font color=#DD0000>')+arr[0]+' <b>'+case128[c][0]+'</b> ('+arr[2]+') '+case128[c][1]+' <b>'+arr[5]+'</b> ['+arr[4]+'] ('+arr[3]+') <b>'+arr[6]+' LR</b>'+(arr[8] ? ' (Причина перевода: '+arr[8]+')' : '')+(arr[2] != arr[3] ? '' : '</font>'); break;
            case 2: r = arr[0]+' <b>Передал на счет <img src=http://img.legendbattles.ru/image/signs/'+arr[7]+'.gif width=15 height=12 border=0 align=absmiddle>клана</b> ('+arr[2]+') <b>'+arr[6]+' LR</b>'+(arr[8] ? ' (Причина перевода: '+arr[8]+')' : ''); break;
            case 3: r = arr[0]+' <b>Получил со счета <img src=http://img.legendbattles.ru/image/signs/'+arr[7]+'.gif width=15 height=12 border=0 align=absmiddle>клана</b> ('+arr[2]+') <b>'+arr[6]+' LR</b>'+(arr[8] ? ' (Причина перевода: '+arr[8]+')' : ''); break;
            case 4: r = arr[0]+' <b>Передал на счет <img src=http://img.legendbattles.ru/image/signs/'+arr[7]+'.gif width=15 height=12 border=0 align=absmiddle>клана</b> ('+arr[2]+') <b>'+arr[6]+' LR</b> со счета <img src=http://img.legendbattles.ru/image/signs/'+arr[5]+'.gif width=15 height=12 border=0 align=absmiddle>клана'+(arr[8] ? ' (Причина перевода: '+arr[8]+')' : ''); break;
        }
