...
function send_out (user) { message_window ('confirm','Исключение персонажа','Вы действительно хотите выгнать персонажа <b>'+user+'</b> из ордена?','accept|cancel','href|main.php?go_out='+user+'&gopers=clan&action=addon') }
function sended_out (user) { message_window ('success','Персонаж исключен','Персонаж <b>'+user+'</b> исключен из состава ордена.<br>С Вашего счета списано <b>200 <img src=images/money.gif></b>.','ok','') }
function got_in (user) { message_window ('success','Персонаж принят','Персонаж <b>'+user+'</b> принят в состав ордена.<br>С Вашего счета списано <b>200 <img src=images/money.gif></b>.','ok','') }
function new_head (user) { message_window ('success','Новый Глава','Персонаж <b>'+user+'</b> успешно назначен на должность Главы Ордена.','ok','') }
function sell_give (user) { message_window ('success','Покупка','Вы удачно купили <b>'+info[0]+'</b>.','ok','') }
function proc_error (number) {
	switch (number) {
		case 1: text = 'Такого персонажа не существует.'; break;
		case 2: text = 'Такого персонажа не существует или <br>персонаж находится в другом ордене.'; break;
		case 3: text = 'Персонаж не находится в ордене.'; break;
		default: text = 'Неизвестная ошибка. Обновите верхний фрейм.'; break;
	}
