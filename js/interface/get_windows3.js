...
function message_window (type,header,message,buttons,address) {
	...
	for (a=0,b=buttons.length;a<b;a++){
			...
			addr = address.split('|');
                if (addr[0] == 'click') {
                    buts += '<td align=center><div id="use_it"><a href="javascript:void(0);" id="use_it" onclick="' + addr[1] + '"><input type=button class=butloc value="ПРИМЕНИТЬ" style="width:120;" border=0></a></div></td>'
                }
                else if (addr[0] == 'href') {
                    buts += '<td align=center><div id="use_it"><a href="javascript:void(0);" id="use_it" onclick="d.location=\'' + addr[1] + '\'"><input type=button class=butloc value="ПРИМЕНИТЬ" style="width:120;" border=0></a></div></td>'
                }
		    }
            else if (buttons[a] == 'cancel') {
                buts += '<td align=center><div id="use_cancel"><a href="javascript:void(0);" id="use_cancel" onclick="close_window();"><input type=button class=butloc value="ОТМЕНА" style="width:120;" border=0></a></div></td>'
            }
            else if (buttons[a] == 'ereage') {
                buts += '<td align=center><div id="ereage"><a href="?" id="ereage" onclick="close_window();"><input type=button class=butloc value="ОК" style="width:120;" border=0></a></div></td>'
            }
			...
			buts += '<td align=center><div id="use_ok"><a href="javascript:void(0);" id="use_ok" onclick="'+addr[0]+'"><img src=interface/ok.gif border=0></a></div></td>' }
                ...
                buts += '<td align=center><div id="use_ok"><a href="javascript:void(0);" id="use_ok" onclick="close_window();"><input type=button class=butloc value="ОК" style="width:120;" border=0></a></div></td>'
            }
	    }
	}
	buts += '</tr></table>';
