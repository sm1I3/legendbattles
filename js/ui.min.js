...
"ui-datepicker-days-cell-over";this.regional=[];this.regional[""]={closeText:"Done",prevText:"Prev",nextText:"Next",currentText:"Today",monthNames:["January","February","March","April","May","June","July","August","September","October","November","December"],monthNamesShort:["Янв","Фев","Март","Апр","Май","Июнь","Июль","Авг","Сен","Окт","Ноя","Дек"],dayNames:["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],dayNamesShort:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],dayNamesMin:["Вс",
"Пн","Вт","Ср","Чт","Пт","Сб"],weekHeader:"Wk",dateFormat:"mm/dd/yy",firstDay:0,isRTL:false,showMonthAfterYear:false,yearSuffix:""};this._defaults={showOn:"focus",showAnim:"show",showOptions:{},defaultDate:null,appendText:"",buttonText:"...",buttonImage:"",buttonImageOnly:false,hideIfNoPrevNext:false,navigationAsDateFormat:false,gotoCurrent:false,changeMonth:false,changeYear:false,yearRange:"c-50:c",showOtherMonths:false,selectOtherMonths:false,showWeek:false,calculateWeek:this.iso8601Week,shortYearCutoff:"+10",
...
//---контекст
(function($){var
menu,shadow,trigger,content,hash,currentTarget;var defaults={menuStyle:{listStyle:'none',padding:'1px',margin:'0px',backgroundColor:'#f5f5f5',border:'1px solid #999',width:'120px'},titleStyle:{fontWeight:'bold',padding:'1px',margin:'0px',backgroundColor:'#CCC',border:'0px solid #999',height:'16px',width:'120px'},itemStyle:{margin:'0px',color:'#000',display:'block',cursor:'pointer',padding:'3px',border:'1px solid #f5f5f5',backgroundColor:'transparent'},itemHoverStyle:{border:'1px solid #AAA',color:'#000',backgroundColor:'#DDD'},leftClick:false,centered:false,eventPosX:'pageX',eventPosY:'pageY',shadow:true,onContextMenu:null,onShowMenu:null,onHideMenu:null};$.fn.contextMenu=function(id,options)
{if(!menu)
{menu=$('<div id="jqContextMenu"></div>').hide().css({position:'absolute',zIndex:'9999'}).appendTo('body').bind('click',function(e){e.stopPropagation();});}
if(!shadow){shadow=$('<div></div>').css({backgroundColor:'#000',position:'absolute',opacity:0.2,zIndex:499}).appendTo('body').hide();}
hash=hash||[];hash.push({id:id,menuStyle:$.extend({},defaults.menuStyle,options.menuStyle||{}),titleStyle:$.extend({},defaults.titleStyle,options.titleStyle||{}),itemStyle:$.extend({},defaults.itemStyle,options.itemStyle||{}),itemHoverStyle:$.extend({},defaults.itemHoverStyle,options.itemHoverStyle||{}),bindings:options.bindings||{},shadow:options.shadow||options.shadow===false?options.shadow:defaults.shadow,onContextMenu:options.onContextMenu||defaults.onContextMenu,onHideMenu:options.onHideMenu||false,onShowMenu:options.onShowMenu||defaults.onShowMenu,eventPosX:options.eventPosX||defaults.eventPosX,eventPosY:options.eventPosY||defaults.eventPosY});var index=hash.length-1;$(this).bind((options.leftClick?'click':($.browser.opera && $.browser.version>10.50?'contextmenu':(!$.browser.opera?'contextmenu':'click'))),function(e)
{if($.browser.opera && $.browser.version<10.51)
{if(!options.leftClick)
{if(!e.ctrlKey)return false;}}
var bShowContext=(!!hash[index].onContextMenu)?hash[index].onContextMenu(e):true;if(bShowContext)
{display(index,this,e,options);return false;}
return true;});return this;};function display(index,trigger,e,options)
{var cur=hash[index];content=$('#'+cur.id).children().clone(true);content.css(cur.menuStyle).find('li').css(cur.itemStyle).hover(function()
{$(this).css(cur.itemHoverStyle);if(document.menuTimer>=0)clearTimeout(document.menuTimer);document.menuTimer=setTimeout(document.menuHide,2000);},function()
{$(this).css(cur.itemStyle);}).find('img').css({verticalAlign:'middle',paddingRight:'2px'});menu.html(content);menu.find('div').css(cur.titleStyle);if(!!cur.onShowMenu)
{r=cur.onShowMenu(e,menu);if(!r)return;menu=r;}
$.each(cur.bindings,function(id,func)
{$('#'+id,menu).bind('click',function(e)
{if(func(trigger,menu,this))hide();});});var
wWidth=$(window).width(),wHeight=$(window).height(),X=e[cur.eventPosX],Y=e[cur.eventPosY];if(options.centered)
{X-=Math.floor(menu.width()/2);Y+=10;}
else
{X+=4;Y+=4;}
if(X+menu.width()>wWidth)X-=X+menu.width()-wWidth+5;if(Y+menu.height()>wHeight)Y-=menu.height();menu.css({'left':X,'top':Y}).show();if(cur.shadow)shadow.css({width:menu.width(),height:menu.height(),left:X,top:Y}).show();$(document).one('click',hide);document.menuHide=hide;document.menuTimer=setTimeout(document.menuHide,2000);}
function hide()
{if(document.menuTimer>=0)clearTimeout(document.menuTimer);menu.hide();shadow.hide();}
