...
function buildKeyboardInputs() {
        ...
        if (inputElems[y]) {
            for (var x = 0; x < inputElems[y].length; x++) {
                if ((inputElems[y][x].nodeName == "TEXTAREA" || inputElems[y][x].type == "text" || inputElems[y][x].type == "password") && inputElems[y][x].className.indexOf("keyboardInput") > -1) {
                    ...
                    keybut.title = "Показать графическую клавиатуру.";
    ...
    checkbox.title = "Переключить на ввод спец символов";
    checkbox.onclick = function () {
        self.VKI_deadkeysOn = this.checked;
        this.nextSibling.nodeValue = " Спец символы: " + ((this.checked) ? "Вкл." : "Выкл.");
        self.VKI_modify("");
        return true;
    }
    label.appendChild(checkbox);
    label.appendChild(document.createTextNode(" Спец символы: " + ((checkbox.checked) ? "Вкл." : "Выкл.")))
    ...
    clearer.appendChild(document.createTextNode("Очистить поле ввода"));
        ...
        for (var x = 0, hasDeadKey = false; x < this.VKI_layout[this.VKI_kt].length; x++) {
            ...
            for (var y = 0; y < this.VKI_layout[this.VKI_kt][x].length; y++) {
                ...
                td.onmouseover = function (e) {
                    if (this.className != "dead" && this.firstChild.nodeValue != "\xa0") this.className += " hover";
                }
                td.onmouseout = function (e) {
                    if (this.className != "dead") this.className = this.className.replace(/ ?(hover|pressed)/g, "");
                }
                td.onmousedown = function (e) {
                    if (this.className != "dead" && this.firstChild.nodeValue != "\xa0") this.className += " pressed";
                }
                td.onmouseup = function (e) {
                    if (this.className != "dead") this.className = this.className.replace(/ ?pressed/g, "");
                }
                ...
                switch (this.VKI_layout[this.VKI_kt][x][y][1]) {
                    ...
                    case "AltGr":
                        ...
                        })(this.VKI_layout[this.VKI_kt][x][y][1]);
                        break;
                    case "Tab":
                        td.onclick = function () {
                            self.VKI_insert("\t");
    ...
    if (window.sidebar || window.opera) {
        this.VKI_keyboard.onmousedown = function () {
            return false;
        }
        this.VKI_keyboard.onclick = function () {
            return true;
        }
    } else this.VKI_keyboard.onselectstart = function () {
        return false;
    }
                ...
                switch (this.VKI_layout[this.VKI_kt][x][y][1]) {
                    ...
                    default:
                        ...
                        if (this.VKI_deadkeysOn) {
                            ...
                            for (key in this.VKI_deadkey) if (key === char) {
                                alive = true;
                                break;
                            }
                        }
