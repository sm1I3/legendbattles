...
if($_GET['Ajax'] == 'yes'){
	if(strlen($_GET['q'])>0){
		...
		$GetUsers = mysqli_query($GLOBALS['db_link'],"SELECT * FROM `user` WHERE `login` LIKE '".preg_replace("/[^a-zA-Zа-яА-Я0-9 _-]/","",iconv("UTF-8","Windows-1251",mysql_real_escape_string($_GET['q'])))."%' ORDER BY `level` DESC LIMIT 7;");
...
if(!empty($_GET['info'])){
	exit(header("Location: /ipers.php?".(iconv("UTF-8","Windows-1251",urldecode($_GET['info']))?iconv("UTF-8","Windows-1251",urldecode($_GET['info'])):urldecode($_GET['info']))));
		}
...
while ($row = mysqli_fetch_assoc($slot)) {
	...
	switch($row['mod_color']){
		case 0: $color="";$ecolor="";break;
		case 1: $color="<font color=#006600>";$ecolor="</font>";break;
		case 2: $color="<font color=#3333CC>";$ecolor="</font>";break;
		case 3: $color="<font color=#993399>";$ecolor="</font>";break;
	}
	$modstat='';
	$mod=explode("|",$row['mod']);
	...
	foreach ($it as $value){
		...
		switch($stat[0]){
			...
			break;
			case 9: $par[2]="|".($stat[1]+$modstat[9]);break;
			case 10: $par[3]="|".$stat[1];break;
			case 27: $par[4]="|".$stat[1];break;
			case 29: $par[5]="|".$stat[1]."|0";break;
		}
		if($row['dd_price']>0 and $row['dd_price']<500){$par[0]=1;}elseif($row['dd_price']>500){$par[0]=2;}else{$par[0]=0;}
		$par[0] = ($par[0]?$par[0]:0);
...
foreach($buffs as $value){
	...
	switch($buff[0]){
			case 1:	$buffname[$buff[0]]="[101,'<b>Мощь</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//Мощь
			case 2: $buffname[$buff[0]]="[102,'<b>Проворность</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//Проворность
			case 3: $buffname[$buff[0]]="[103,'<b>Везение</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//Везение
			case 4: $buffname[$buff[0]]="[104,'<b>Здоровье</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//здоровье
			case 5: $buffname[$buff[0]]="[105,'<b>Разум</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//Разум
			case 6: $buffname[$buff[0]]="[999,'<b>Сноровка</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//сноровка
			case 7: $buffname[$buff[0]]="[111,'<b>Удар</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//удар
			case 8: $buffname[$buff[0]]="[110,'<b>КБ</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//кб
			case 9: $buffname[$buff[0]]="[112,'<b>Пробой брони</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//пробой брони
			case 10: $buffname[$buff[0]]="[106,'<b>Уловка</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//уловка
			case 11: $buffname[$buff[0]]="[107,'<b>Точность</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//точность
			case 12: $buffname[$buff[0]]="[108,'<b>Сокрушение</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//сокрушение
			case 13: $buffname[$buff[0]]="[109,'<b>Стойкость</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//стойкость
			case 14: $buffname[$buff[0]]="[100,'<b>Ангел</b> +".$buffcount[$buff[0]]."% (еще ".$time[$buff[0]].")'],";break;//арт зелье
			case 15: $buffname[$buff[0]]="[113,'<b>Наблюдательность</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//зелье наблюдательности
			case 16: $buffname[$buff[0]]="[10,'<b>Странник</b> +".$buffcount[$buff[0]]." (еще ".$time[$buff[0]].")'],";break;//странник
...
$i=1;
while($i<=16){$effects .= (($buffname[$i]!='')?$buffname[$i]:'');$i++;}
$effects .= (($traw)?$traw:'');
...
".str_replace("\n","<br/>",$pers['about'])."</div></td><td class=\"right_middle\"></td></tr><tr><td class=\"left_bot\"></td><td class=\"center_bot\"></td><td class=\"right_bot\"></td></tr></table>";
if(accesses($WatchUser['id'],'pvu')){
...
echo'<HEAD><LINK href="./css/game.css?'.$FilesVersion.'" rel=STYLESHEET type=text/css><LINK href="./ch/chat.css?'.$FilesVersion.'" rel=STYLESHEET type=text/css><meta content="text/html; charset=windows-1251" http-equiv=Content-type><META Http-Equiv=Cache-Control Content=no-cache><meta http-equiv=PRAGMA content=NO-CACHE><META Http-Equiv=Expires Content=0><SCRIPT src="/js/ajax.js?'.$FilesVersion.'"></SCRIPT>
