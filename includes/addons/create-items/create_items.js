...
switch(type){
	case '': second='';break;
    case 'weapon':
        second = '<font class weaponchdis>&nbsp;Второе оружие ( да <input name="wtor" type="radio" value="1" onClick="writeparams2();" /> ) ( нет <input name="wtor" type="radio" value="0" onClick="writeparams2();" /> )<input type=hidden name="block" value="0" /></font>';
        break;
    case 'shield':
        second = '<select name="block" width=50><option value="0" selected="selected">Блок точек</option><option value="40">1 точка</option><option value="70">2 точки</option><option value="90">3 точки</option></select>';
        break;
}
el.innerHTML = second;
}
