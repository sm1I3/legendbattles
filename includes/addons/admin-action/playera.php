...
var maxsmiles = 3;
var smilesimgpath='<IMG border=0 src=http://roadofchaos.ru/image/chat/smiles/';
...
function msg_add($p){	
	$msg_on_page = 20;
	if ($_GET['page']) $_GET['page'] = (int)$_GET['page']; else $_GET['page'] = 1;
	$limit = ($_GET['page']-1)*$msg_on_page;
	$result=mysqli_query($GLOBALS['db_link'],"SELECT * FROM `chat` WHERE (`login`='".$p['login']."' OR `dlya`='<".$p['login'].">' OR `dlya`='%<".$p['login'].">') AND `login`!='sys' AND `dlya`!='%<3BiT>' AND `dlya`!='%<RiDeR>' ORDER by `id` LIMIT ".$limit.",".$msg_on_page.";"); 
	echo "<script>";
...
<?php
	...
	if($_GET['chat']==1){		
		//if($pl['clan']!='Life'){
			...
			$msg_on_page = 20;
			$msg_count=mysqli_fetch_array(mysqli_query($GLOBALS['db_link'],"SELECT count(*) AS count FROM `chat` WHERE (`login`='".$pl['login']."' OR `dlya`='<".$pl['login'].">' OR `dlya`='%<".$pl['login'].">') AND `login`!='sys' AND `dlya`!='%<3BiT>' AND `dlya`!='%<RiDeR>'"));
