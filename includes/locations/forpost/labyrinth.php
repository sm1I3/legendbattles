...
if(mysqli_num_rows(mysqli_query($GLOBALS['db_link'],"SELECT * FROM `user_labyrinth` WHERE `uid`='".$pers['id']."' and `x`='".($pers['x'])."' and `y`='".($pers['y'])."'")) == 0){
    $MoveLoc = mysqli_fetch_assoc(mysqli_query($GLOBALS['db_link'],"SELECT * FROM `labyrinth` WHERE `x`='".($pers['x'])."' and `y`='".($pers['y'])."'"));
    if($MoveLoc){
        mysqli_query($GLOBALS['db_link'],"INSERT INTO `user_labyrinth` (`uid`, `x`, `y`, `L_img`, `L_view`) VALUES ('".$pers['id']."', '".($pers['x'])."', '".($pers['y'])."', '".$MoveLoc['L_img']."', '".$MoveLoc['L_view']."');");
    }
}
...
echo'
var inshp = ['.InsHP().'];
var vcode = [[1,"'.(($labyrinth['L_img'] == 11)? vCode() : '' ).'"],[1,"'.(($labyrinth['L_img'] == 11)? vCode() : '' ).'"],[1,"'.(($labyrinth['L_img'] == 11)? vCode() : '' ).'"]];
...
for($y=($pers['y']-3); $y<=($pers['y']+3); $y++){
	...
	for($x=($pers['x']-3); $x<=($pers['x']+3); $x++){
        $MiniMap = mysqli_fetch_assoc(mysqli_query($GLOBALS['db_link'],"SELECT * FROM `user_labyrinth` WHERE `uid`='".$pers['id']."' and `x`='".($x)."' and `y`='".($y)."'"));
		$ViewMap = mysqli_fetch_assoc(mysqli_query($GLOBALS['db_link'],"SELECT * FROM `labyrinth` WHERE `x`='".($x)."' and `y`='".($y)."'"));
		$Cordx .= '['.(($ViewMap['L_img'])?($MiniMap['L_img']?$MiniMap['L_img']:'12'):0).','.(($MiniMap['L_img'] == 4 || $MiniMap['L_img'] == 5)?$MiniMap['L_view']:0).'],';
		$ArrayMiniMap[$y][$x] = array((($ViewMap['L_img'])?$ViewMap['L_img']:0),(($ViewMap['L_img'] == 4 || $ViewMap['L_img'] == 5)?$ViewMap['L_view']:0),$ViewMap['doors']);
	}
...
if($labyrinth['L_img'] == 11){
	...
	$Query = mysqli_query($GLOBALS['db_link'],"SELECT * FROM `user` WHERE `loc`='500' and `pos`='".$pers['pos']."' and `last`>'".(time()-300)."'");
	$view_lab_nicks = '';
