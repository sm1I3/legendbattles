	...
	switch($_GET['act']){
		case'10':
			switch($_GET['go']){
				...
				break;	
				case'up':
					$LocID = mysqli_result(mysqli_query($GLOBALS['db_link'],"SELECT `go_id` FROM `locations` WHERE `id`='".$pers['loc']."'"),0);
					if($LocID){
						mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `loc`='".$LocID."' WHERE `id`='".$pers['id']."'");
						header("Location: /main.php");
					}
		...
		case '11':
			...
			$val_go=varcheck($_GET['go']);
				mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `loc`='".mysqli_real_escape_string($val_go)."' WHERE `id`='".$pers['id']."'");
				header("Location: /main.php");
			}
		break;		
	}
