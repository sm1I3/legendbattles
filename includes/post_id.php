...
if($_POST['post_id']=='1' and in_array($_POST['vcode'],$_SESSION['vcodes'])){
	...
	if($pers['clan_id']!='none'){
		...
		if(intval($_POST['param'])==100){
			...
			if($clsql['cl_up']>=1){
				switch(intval($_POST['param'])){
					...
					case 4: $sql="SET cl_zdorov=cl_zdorov+1,cl_up=cl_up-1 WHERE clan_id='".$pers['clan_id']."' ";$msg="<font class=travma style='color:red'><b>Вы удачно приобрели клановое усиление +1 здоровья.";$err=0;break;
		...
		if($pers['baks']<$sum){$err=1;$msg="<font class=travma style='color:red'><b>Недостаточно денег.";}
	}
	if($err==0){
...
if($_POST['post_id']=='47' and in_array($_POST['vcode'],$_SESSION['vcodes'])){
	...
	if($warn == 0){
		if($pers['nv']<1000){echo "<script>alert('У вас нехватает денег');</script>";}
		else{
