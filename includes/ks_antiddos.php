...
// прочистка списка	
	function clean_list()
	{
		foreach($this->iplist as $ip=>$times)
		{
			...
			{
				if ($times[$i]+$this->seconds_limit>$this->now) { // нашли
					$actual_ptr = $i;
					break;
				}
			}
	...
	function read_iplist()
	{
		...
		if (false===$this->iplist) { // создаем переменную в шареде
			$this->iplist = array();
...
if (strrchr($_SERVER['SCRIPT_NAME'],'/')==='/ks_antiddos.php') { // вызов напрямую из браузера
