	...
	switch($type){
		case 1:
		 ...
		 foreach($lic as $value){
			...
			if($licnum[0]==1 and $licnum[1]>time()){$licen=0.99;}
		 }
		 break;
		 case 2:
		 $licen=0;
		 ...
		 foreach($lic as $value){
			$licnum=explode("@",$value);
			if($licnum[0]==2 and $licnum[1]>time()){$licen=1;}
		 }
		 break;
	 }
	 return $licen;
...
if($hp<($hpa*0.5)and $mp>5 and $zn>1){$s[mag]="320_0@";}
if($zn>$sil and $mpa>50 and $mp>10){$s2="_2_100@";}else{$s2="_0_0@";}
if($od<140){$s[ud]=rand(0,3).$s2;}else{$s1=rand(0,2);$s[ud]=$s1.$s2.($s1+1).$s2;}
...
if($rooms['room']!=''){$room=", $rooms[room]";}else if($rooms['city']!=""){$city="$rooms[city], ";}
...
elseif(strlen($count.$ct)<4){$wdth = 110*1.1;}
elseif(strlen($count.$ct)<5){$wdth = 120*1.1;}
elseif(strlen($count.$ct)<6){$wdth = 130*1.1;}
elseif(strlen($count.$ct)<7){$wdth = 140*1.1;}
elseif(strlen($count.$ct)<8){$wdth = 150*1.1;}
elseif(strlen($count.$ct)<9){$wdth = 160*1.1;}
...
function calchp()
{
	...
	$trw=affect($pl[affect],3);foreach ($trw as $key => $val){$s[$key]+=$val;}
...
if($pl['loc']=='' and $pl['id']){mysqli_query($GLOBALS['db_link'],"update `user` set `loc`='1' where `id`='".$pl['id']."' limit 1;");}
return $pl;
}
...
if($id==28){$loc= mysqli_fetch_assoc(mysqli_query($GLOBALS['db_link'],"SELECT map.go_id, map.but FROM map WHERE coord='$pos' LIMIT 1;"));$loc[folder]="maps";}else{
...
$mps=$pl['mp_all']/$pl['mps'];
 if(time()>=$pl['chp']){$curhp=$pl['hp_all'];}else{$curhp=$pl['hp_all']-(($pl['chp']-time())*$hps);}
 if(time()>=$pl['cmp']){$curmp=$pl['mp_all'];}else{$curmp=$pl['mp_all']-(($pl['cmp']-time())*$mps);}
if($pl['hp_all']!=$pl['hp'] or $pl['mp_all']!=$pl['mp']){mysqli_query($GLOBALS['db_link'],'UPDATE user SET hp='.AP.$curhp.AP.', mp='.AP.$curmp.AP.' WHERE id='.AP.$pl['id'].AP.' LIMIT 1;');}}}
...
case 1: $ud=explode("-",$stat[1]);$par[1]="$ud[0]";$par[2]="$ud[1]";break;
...
function getPF($id){
	...
	if(in_array($id,$p)){return 1;}else{return 0;}
}
...
function ignor_add($nic){$_SESSION['ignor'][$nic]=1;}
function ignor_rem($nic){unset($_SESSION['ignor'][$nic]);}
...
if($row['slot']>0 and $row['type']!='w20'){
	...
	foreach ($item as $value) {
		$k=1;
		$stat=explode("@",$value);
		if(in_array ($stat[0], $t) and $stat[0]!='expbonus' and $stat[0]!='massbonus'){$par[$stat[0]]='';continue;}
		if($stat[0]==1){
			$tmp=explode("-",$stat[1]);
			switch($tw){
	...
	if(mysqli_num_rows($podsql)>0){
		while($podrow = mysqli_fetch_assoc($podsql)){
			switch($podrow[podarok]){
				case 9999: $updmg[0]+=15;$updmg[1]+=25;$uphp+=225;$upmp+=175;$upstat+=5;break;
				case 9998: $updmg[0]+=10;$updmg[1]+=15;$uphp+=150;$upmp+=100;$upstat+=3;break;
				case 9997: $updmg[0]+=10;$updmg[1]+=15;$uphp+=100;$upmp+=50;$upstat+=1;break;
				case 9996: $updmg[0]+=5;$updmg[1]+=10;$uphp+=50;$upmp+=25;$upstat+=0;break;
			}
		}
		for($i=29;$i<=34;$i++){
			$par[$i]+=$upstat;
...
foreach($perk as $key=>$val){if($val==''){$perk[$key]=0;}$perk[$key]=$val;}
$pt[5]+=$perk[19]*30;
$pt[6]+=$perk[0]*30;
$pt[7]+=$perk[5]*30;
$pt[8]+=$perk[15]*30;
...
function addlic($login,$item,$type){
		...
		case 1:
			if($bt==0){
				...
				foreach($lic as $value){
					...
					if($licens[0]==1){
						if($licens[1]<time()){$licens[1]=time()+$time;}
						else{$licens[1]+=$time;}
	...
	$typetolog .= '@23'; $abouttolog .= '@'.$msg[msg];
...
function zelused($id,$login,$loc){
	...
	while($i<=4){
		if($buffs[$i]!=''){
			...
			if($it[num_a]==14){
				$buff.="$it[num_a]@$it[effect]@".(time()+7200)."|";$i=99;
			}else{
				$buff.="$it[num_a]@$it[effect]@".(time()+3600)."|";$i=99;
			}
		}
	}
	if($i==99){
...
function testcompl(){
	...
	while ($row = mysqli_fetch_assoc($sql)) {
		$it = explode("|",$row['need']);
		if($row['slot']==5){$el+=1;}
		if($row['slot']==5 and $el>$st[3]){updateslot(0,$row['id_item'],$pl['id'],0);continue;}
		foreach ($it as $value) {
		$stat=explode("@",$value);
		if($stat[0]==72){$stat[1]=$row['level'];}
		if($st[$stat[0]]<$stat[1]){if($row['slot']==4){$st[3]=0;}$s[]="`id_item`='".$row['id_item']."'";}
		}
	}
	if($s!=''){
		$s=implode(" or ",$s);
...
function test_affect($aff){
	...
	foreach ($aff as $val){
		$par=explode("@",$val);
		if($par[1]<time() and $par[1]!=''){continue;}
		$a[]=$val;
	}
	$aff=implode("|",$a);
	return $aff;
...
$ret="";
$trwrand=rand(0,100);
if($pl['sklon']==5 and $trwrand<66 and $pl['ability']>0 and $pl['lastability']<time()){$trwtime=2;}else{$trwtime=1;}
if($persent<40){$persent=40;}
if($persent<100)
{
	$rand=rand(0,$persent);
	if($rand<=40){
...
function endbat($id,$t,$k4){
	$usr=player();
	$side=array(1=>2,2=>1); $order=array(1=>"ASC",2=>"DESC");
	...
	if(mysqli_num_rows($sql)!=0){
		while ($p = mysqli_fetch_assoc($sql)) {
			...
			$wins=explode("|",$p[wins]); if($p[bt]==1){$p[bt]=2;}
			if($p['side']==$t[0]){
				$SideWin = $t[0];
				if($p['invisible']<time()){
		...
		mysqli_query($GLOBALS['db_link'],"update instant set level=level+1 where uid='$usr[id]'");
				...
				if($t[999]==1){
					if($p['travma']==100){$ftr=100;}else{$ftr=60;}
					if($p['invisible']<time()){
...
switch($pl[level]){
case 0: $a=array(1=>12,1,3,8);break;
case 1: $a=array(1=>15,1,6,12);break;
case 2: $a=array(1=>18,2,9,16);break;
case 3: $a=array(1=>22,2,13,21);break;
case 4: $a=array(1=>27,3,17,25);break;
case 5: $a=array(1=>32,3,22,30);break;
case 6: $a=array(1=>36,4,27,35);break;
case 7: $a=array(1=>42,4,33,41);break;
case 8: $a=array(1=>49,5,40,48);break;
case 9: $a=array(1=>56,5,46,54);break;
case 10: $a=array(1=>65,6,55,62);break;
case 11: $a=array(1=>73,6,63,69);break;
case 12: $a=array(1=>80,7,73,81);break;
case 13: $a=array(1=>90,7,88,93);break;
case 14: $a=array(1=>102,8,108,109);break;
case 15: $a=array(1=>117,8,122,125);break;
case 16: $a=array(1=>130,9,136,141);break;
case 17: $a=array(1=>145,9,160,163);break;
case 18: $a=array(1=>160,10,180,181);break;
case 19: $a=array(1=>180,10,205,199);break;
case 20: $a=array(1=>205,11,220,213);break;
case 21: $a=array(1=>225,11,235,225);break;
case 22: $a=array(1=>246,12,255,240);break;
case 23: $a=array(1=>270,12,273,260);break;
case 24: $a=array(1=>295,13,291,280);break;
case 25: $a=array(1=>325,14,315,305);break;
}
...
function exp_level($level){
	...
	$exp=round($exp);
	switch($level){
		case 0: $arr=array("exp"=>$exp,"ma"=>10,"ex"=>$ex,"frs"=>3,"nv"=>100,"nav"=>0,"mum"=>3,"bum"=>4);break;
		case 1: $arr=array("exp"=>$exp,"ma"=>12,"ex"=>$ex,"frs"=>3,"nv"=>200,"nav"=>1,"mum"=>3,"bum"=>4);break;
		case 2: $arr=array("exp"=>$exp,"ma"=>16,"ex"=>$ex,"frs"=>4,"nv"=>200,"nav"=>0,"mum"=>4,"bum"=>5);break;
		case 3: $arr=array("exp"=>$exp,"ma"=>20,"ex"=>$ex,"frs"=>5,"nv"=>250,"nav"=>1,"mum"=>4,"bum"=>4);break;
		case 4: $arr=array("exp"=>$exp,"ma"=>24,"ex"=>$ex,"frs"=>5,"nv"=>500,"nav"=>0,"mum"=>5,"bum"=>5);break;
		case 5: $arr=array("exp"=>$exp,"ma"=>40,"ex"=>$ex,"frs"=>4,"nv"=>500,"nav"=>1,"mum"=>5,"bum"=>5);break;
		case 6: $arr=array("exp"=>$exp,"ma"=>52,"ex"=>$ex,"frs"=>6,"nv"=>600,"nav"=>0,"mum"=>6,"bum"=>6);break;
		case 7: $arr=array("exp"=>$exp,"ma"=>56,"ex"=>$ex,"frs"=>7,"nv"=>550,"nav"=>1,"mum"=>7,"bum"=>7);break;
		case 8: $arr=array("exp"=>$exp,"ma"=>64,"ex"=>$ex,"frs"=>7,"nv"=>600,"nav"=>0,"mum"=>6,"bum"=>6);break;
		case 9: $arr=array("exp"=>$exp,"ma"=>76,"ex"=>$ex,"frs"=>9,"nv"=>600,"nav"=>1,"mum"=>9,"bum"=>8);break;
		case 10: $arr=array("exp"=>$exp,"ma"=>80,"ex"=>$ex,"frs"=>8,"nv"=>700,"nav"=>0,"mum"=>8,"bum"=>7);break;
		case 11: $arr=array("exp"=>$exp,"ma"=>104,"ex"=>$ex,"frs"=>7,"nv"=>700,"nav"=>1,"mum"=>10,"bum"=>12);break;
		case 12: $arr=array("exp"=>$exp,"ma"=>120,"ex"=>$ex,"frs"=>10,"nv"=>800,"nav"=>0,"mum"=>15,"bum"=>12);break;
		case 13: $arr=array("exp"=>$exp,"ma"=>136,"ex"=>$ex,"frs"=>12,"nv"=>800,"nav"=>1,"mum"=>20,"bum"=>16);break;
		case 14: $arr=array("exp"=>$exp,"ma"=>150,"ex"=>$ex,"frs"=>15,"nv"=>1000,"nav"=>0,"mum"=>14,"bum"=>16);break;
		case 15: $arr=array("exp"=>$exp,"ma"=>170,"ex"=>$ex,"frs"=>13,"nv"=>1000,"nav"=>1,"mum"=>14,"bum"=>16);break;
		case 16: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>15,"nv"=>1500,"nav"=>0,"mum"=>24,"bum"=>22);break;
		case 17: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>15,"nv"=>1900,"nav"=>1,"mum"=>20,"bum"=>18);break;
		case 18: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>20,"nv"=>2000,"nav"=>0,"mum"=>25,"bum"=>18);break;
		case 19: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>25,"nv"=>3000,"nav"=>1,"mum"=>15,"bum"=>14);break;
		case 20: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>20,"nv"=>3500,"nav"=>0,"mum"=>15,"bum"=>12);break;
		case 21: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>21,"nv"=>4000,"nav"=>1,"mum"=>20,"bum"=>15);break;
		case 22: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>24,"nv"=>5000,"nav"=>0,"mum"=>18,"bum"=>20);break;
		case 23: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>25,"nv"=>5000,"nav"=>1,"mum"=>18,"bum"=>20);break;
		case 24: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>30,"nv"=>10000,"nav"=>1,"mum"=>24,"bum"=>25);break;
		case 25: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>30,"nv"=>10000,"nav"=>1,"mum"=>24,"bum"=>25);break;
		case 26: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>30,"nv"=>10000,"nav"=>1,"mum"=>24,"bum"=>25);break;
		case 27: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>30,"nv"=>10000,"nav"=>0,"mum"=>24,"bum"=>25);break;
		case 28: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>30,"nv"=>10000,"nav"=>0,"mum"=>24,"bum"=>25);break;
		case 29: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>30,"nv"=>10000,"nav"=>1,"mum"=>24,"bum"=>25);break;
		case 30: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>30,"nv"=>10000,"nav"=>0,"mum"=>24,"bum"=>25);break;
		case 31: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>30,"nv"=>10000,"nav"=>0,"mum"=>24,"bum"=>25);break;
		case 32: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>30,"nv"=>10000,"nav"=>0,"mum"=>24,"bum"=>25);break;
		case 33: $arr=array("exp"=>$exp,"ma"=>180,"ex"=>$ex,"frs"=>30,"nv"=>10000,"nav"=>1,"mum"=>24,"bum"=>25);break;
...
function ins_bot_logovo($bot_id,$bot_kolvo,$fid){
	...
	while($i<$bot_kolvo[$k]){
		...
		$i++;
	//	echo'bot|';
		if($i<$bot_kolvo[$k] or $k<$n-1){$insert.=$ins.",";}else{$insert.=$ins;}
	}
//	echo'test|';
	$k++;$i=0;
}	
...
function BotAttack($player){
	...
	if(mysqli_num_rows($query)>0){
			...
			if($player['pos']=='1_2'){$randtime=rand(600,720);}
			$sk='kgTvx2WrEZ';
...
function trane_bots($bots_ids,$fid){
	...
	for($i = 0; $i < count($bots_ids); $i++){
		...
		$bot['fight'] = 0;
		$ins="('".$bot['damage_mods']."','".$bot['access']."','".$bot['type']."','".$bot['login']."','".$bot['pass']."','".$bot['email']."','".$bot['useaction']."','".$bot['icq']."','".$bot['name']."','".$bot['country']."','".$bot['city']."','".$bot['bday']."','".$bot['url']."','".$bot['sex']."','".$bot['thotem']."','".$bot['bdaypers']."','".$bot['ip']."','".$bot['filt']."','".$bot['pcid']."','".$bot['last']."','".$bot['lastbattle']."','".$bot['wait']."','".$bot['chcolor']."','".$bot['loc']."','".$bot['pos']."','".$bot['level']."','".$bot['clan_id']."','".$bot['clan']."','".$bot['clan_d']."','".$bot['clan_gif']."','".$bot['clan_accesses']."','".$bot['clan_status']."','".$bot['clan_check']."','".$bot['sklon']."','".$bot['nv']."','".$bot['dd']."','".$bot['baks']."','".$bot['obraz']."','".$bot['f_obraz']."','".$bot['obr_col']."','".$bot['sila']."','".$bot['lovk']."','".$bot['uda4a']."','".$bot['zdorov']."','".$bot['znan']."','".$bot['mudr']."','".$bot['ustal']."','".$bot['od']."','".$bot['bl']."','".$bot['free_stat']."','".$bot['hp']."','".$bot['hp_all']."','".$bot['mp']."','".$bot['mp_all']."','".$bot['hps']."','".$bot['mps']."','".$bot['chp']."','".$bot['cmp']."','".$bot['st']."','".$bot['affect']."','".$bot['umen']."','".$bot['perk']."','".$bot['fr_bum']."','".$bot['fr_mum']."','".$bot['nav']."','".$bot['battle']."','".$bot['side']."','".$bot['fight']."','".$bot['sleep']."','".$bot['block']."','".$bot['prison']."','".$bot['finblock']."','".$bot['addon']."','".$bot['about']."','".$bot['dmg']."','".$bot['exp']."','".$bot['wins']."','".$bot['mov']."','".$bot['obnul']."','".$bot['licens']."','".$bot['options']."','".$bot['semija']."','".$bot['a_m']."','".$bot['sign']."','".$bot['minex']."','".$bot['miney']."','".$bot['waiter']."','".$bot['sp7']."','".$bot['forum_accesses']."','".$bot['forum_smiles']."','".$bot['forum_lastmsg']."','".$bot['firstlogin']."')";
		if($i < (count($bots_ids)-1)){$insert.=$ins.",";}else{$insert.=$ins;}
	}
	return $insert;
}
