...
if ($get_id == 15) {
    ...
    list($y, $x) = explode("_", $player['pos'], 2);
    if ($x = 1005 && $y = 1001) {
...
if ($get_id == 42) {
    ...
    if ($act == 2 and $num == 0) {
        mysqli_query($GLOBALS['db_link'], 'UPDATE user SET pos=' . AP . $cx . "_" . $cy . AP . ', nv=nv+1, wait=' . AP . (time() + $mtime) . AP . ' WHERE id=' . AP . $player['id'] . AP . ' LIMIT 1;');
        mysqli_query($GLOBALS['db_link'], 'INSERT INTO unground (coord,x,y,image) VALUES (' . AP . $cx . "_" . $cy . AP . ',' . AP . $cx . AP . ',' . AP . $cy . AP . ',' . AP . rand(1, 4) . AP . ');');
    }
    mysqli_query($GLOBALS['db_link'], "UNLOCK TABLES;");
}
