...
if($_GET['buy']==1 and intval($_POST['rid'])!=""){include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/alhim/alhim_buy".".php");}
if($_GET['create']==1 and intval($_POST['rid'])!=""){include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/alhim/alhim_create".".php");}
$player = player();	
$pt=allparam($player);
echo'
...
<table border=0 cellpadding=4 cellspacing=1 bordercolor=#e0e0e0 align=center class="smallhead" width=100%>
	...
	$recipes = mysqli_query($GLOBALS['db_link'],"SELECT * FROM `alhim` WHERE `nav`<='".$pt[68]."' ORDER BY `nav`"); //2 - находим рецепты которые соответствуют навыку игрока
	...
	while ($row = mysqli_fetch_assoc($recipes)){
			...
			<td align=center rowspan=2>
				<b>'.$rec_inf['name'].'</b><br>Долговечность: '.$rec_inf['dolg'].'/'.$rec_inf['dolg'].'<br>Количество создаваемых зелий: <b>'.$row['col'].'</b> шт.
			</td>
			<td align=center>';
			$i=0;
			foreach ($reg as $val){			 
			...
			if($player['alhim_rec']=="0"){$alhim_rec="||||";}else{$alhim_rec=explode("|",$player['alhim_rec']);}
			echo'</td></tr>
			<tr class=nickname bgcolor=white><td align=center>
			'.((in_array($row['id'],$alhim_rec))
			?
			'<form method=post action="main.php?hospi_sel=3&create=1"><input type="submit" class="lbut" value="Создать" /><input type=hidden name=rid value="'.$row['id'].'"/></form>'
			:
			'<form method=post action="main.php?hospi_sel=3&buy=1"><button type="submit">Купить рецепт [ '.lr($row['price']).' ]</button><input type=hidden name=rid value="'.$row['id'].'"/></form>').'
			</td>
			</tr>
			';//6.1
	}
