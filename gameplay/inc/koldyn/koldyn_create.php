<?
	...
	if(mysqli_num_rows($chrecipe)>0){
		...
		if($pt[75]>=$chrecipe['nav'] and in_array($chrecipe['id'],$koldyn_rec)){
			...
			if($IT!='' and $err=="0"){
				...
				$message=5;include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/koldyn/koldyn_messages".".php");
			}
			else if($err=="1"){$message=6;include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/koldyn/koldyn_messages".".php");}
			else if($IT==''){$message=7;include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/koldyn/koldyn_messages".".php");}
		}
		else{
			(($pt[75]<$chrecipe['nav'])?($message=2):($message=8));
