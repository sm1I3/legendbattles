...
while($bot = mysqli_fetch_assoc($bots)){
		$chbot=mysqli_fetch_assoc(mysqli_query($GLOBALS['db_link'],"SELECT * FROM `bot_drop`  WHERE `bot_id`='".$bot['id']."' LIMIT 1;"));	
			echo'
				...
				<tr align=center class=nickname><td><img src="http://efilands.ru/image/obrazy/' . $bot['obraz'] . '" width="100" height="100" border="0" align="absmiddle" onmouseover="tooltip(this,\'<b>' . $bot['login'] . ' [ Уровень ' . $bot['level'] . ' ]</b>\')" onmouseout="hide_info(this)"></td></tr><hr>
				';
				...
				foreach($itemsin as $val){
					if($val!=''){
						...
						<tr>							
							<td>
							<form>
							<img src="http://efilands.ru/image/weapon/'.$name['gif'].'" width="20" height="20" border="0" align="absmiddle" onmouseover="tooltip(this,\'<b>'.$name['name'].'</b>\')" onmouseout="hide_info(this)">  '.$name['name'].' <a href="/iteminfo.php?'.$name['name'].'" target="_blank"><img src="http://efilands.ru/image/chat/info1.gif" width="11" height="12" border="0" align="absmiddle"></a>							
							</form>
							</td>
						</tr>';
					}
