...
echo 
'
<div class="block referal">
	<div class="header">
		<span>Ваши рефералы</span>
	</div>
	<div class="content">
	Ваша реферальная ссылка:<b> http://www.legendbattles.ru/ref.php?' . $player['login'] . '</b>
...
<table border=0 cellpadding=4 cellspacing=1 bordercolor=#e0e0e0 align=center class="smallhead" width=100%>';
			...
			<tr class=freetxt bgcolor=white>
				...
				<td align=center colspan=2>
					...
					if($ref['items_'.$i]!='||||'){
                        ...
                        echo 'Бонусные вещи: нет';
                    }
					echo'
				</td>
				<td align=center><b>Деньги:</b> ' . lr($LR['bonus_' . $i]) . '</td>
				<td align=center><b>Изумруд:</b> ' . $DLR['bonus_' . $i] . ' <img src="http://img.legendbattles.ru/razdor/emerald.png" width=12 height=12></td>
			</tr>';
		...
		<tr class=nickname bgcolor=#EAEAEA>
			...
			<td align=center><b>Получено Деньги</b></td>
			<td align=center><b>Получено Изумруд</b> <img src="http://img.legendbattles.ru/razdor/emerald.png" width=14 height=14></td>
		</tr>';
		...
		while($referal = mysqli_fetch_assoc($referals)){
			...
			if($_POST['getbonus_ref']){
				...
				if($u==$reflvl['id'] and $referal['bonus']==0 and $reflvl['level']>=15){
					...
					if($dd or $lr or $insprem or $ref['items_'.$b]!='||||'){			
						if(mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET ".$insprem."`baks`=`baks`+'".$dd."',`nv`=`nv`+'".$lr."' WHERE `id`='".$player['id']."';") and mysqli_query($GLOBALS['db_link'],"UPDATE `ref_system` SET `bonus`='1',`bonus_dlr`=`bonus_dlr`+'".$dd."',`bonus_lr`=`bonus_lr`+'".$lr."' WHERE `who_id`='".$player['id']."' AND `ref_id`='".$referal['ref_id']."' LIMIT 1;")){
							$referal['bonus']=1;
							$referal['bonus_lr']+=$lr;
							...
							if($dd){
                                $ms = "parent.frames['chmain'].add_msg('<font class=chattime>&nbsp;" . date("H:i:s") . "&nbsp;</font> <font color=000000><font color=#000000><b>Системная информация.</b></font> Изумруд: <b>" . $dd . "<b></font><BR>'+'');";
								chmsg($ms,$player['login']);
							}
							if($lr){
                                $ms = "parent.frames['chmain'].add_msg('<font class=chattime>&nbsp;" . date("H:i:s") . "&nbsp;</font> <font color=000000><font color=#000000><b>Системная информация.</b></font> : <b>" . lr($lr) . "</b></font><BR>'+'');";
								chmsg($ms,$player['login']);
							}
							if($insprem){
								switch($ACCTYPE['bonus_'.$b]){
                                    case 2:
                                        $msg = "SILVER аккаунт на " . $ACCTIME['bonus_' . $b] . " дней";
                                        break;
                                    case 3:
                                        $msg = "GOLD аккаунт на " . $ACCTIME['bonus_' . $b] . " дней";
                                        break;
                                    case 4:
                                        $msg = "VIP аккаунт на " . $ACCTIME['bonus_' . $b] . " дней";
                                        break;
								}
			...
			if($bonus_ref){
                ...
                echo '<font color=red><b>недоступно</b></font>';
            }
			echo'</b></td>
			<td align=center><b>'.lr($referal['bonus_lr']).'</b></td>
			<td align=center><b>'.$referal['bonus_dlr'].'</b> <img src="http://img.legendbattles.ru/razdor/emerald.png" width=14 height=14></td>
			';
		}
...
if($_POST['getbonus']){
	...
	if(($b==5 or $b==10 or $b==15) and $player['ref_bonus_'.$b] == 0 and $refbig>=$b){
		...
		if($ref['items_'.$b]==''){$ref['items_'.$b]='||||';}
		if($dd or $lr or $insprem or $ref['items_'.$b]!='||||'){			
			if(mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET ".$insprem."`baks`=`baks`+'".$dd."',`nv`=`nv`+'".$lr."',`ref_bonus_".$b."`='1' WHERE `id`='".$player['id']."';")){
				...
				if($lr){
                    $ms = "parent.frames['chmain'].add_msg('<font class=chattime>&nbsp;" . date("H:i:s") . "&nbsp;</font> <font color=000000><font color=#000000><b>Системная информация.</b></font> Денег: <b>" . lr($lr) . "</b></font><BR>'+'');";
					chmsg($ms,$player['login']);
				}
				if($insprem){
					switch($ACCTYPE['bonus_'.$b]){
...
echo'
		<div class="block">
			...
			<table border=0 cellpadding=4 cellspacing=1 bordercolor=#e0e0e0 align=center class="smallhead" width=100%>
				...
				<td align=center width=20%>
					...
					от полученных Деньги за уровень<br>
				...
				for($i=5;$i<=15;$i+=5){
					...
					<td align=center  width=20%>
						...
						-----------------------------<br>
						<b>Изумруд:</b> ' . $DLR['bonus_' . $i] . ' <img src="http://img.legendbattles.ru/razdor/emerald.png" width=14 height=14><br><b>Деньги:</b> ' . lr($LR['bonus_' . $i]) . '<br>
						';
						if($ref['items_'.$i]==''){$ref['items_'.$i]='||||';}
			...
			<tr>
				<td width=100% align=center bgcolor=#EAEAEA colspan=4>
					<font class=proceb style="font-size: 16px;"><b>Бонус:</b>&nbsp;<font class=proce style="font-size: 16px;"><b>' . $ref['money_dlr_bonus'] . '%</b></font>&nbsp;от купленных рефералом Изумруд <img src="http://img.legendbattles.ru/razdor/emerald.png" width=14 height=14> (Средства будут зачислены Вам автоматически.)</font>
				</td>
			</tr>
			</table>
		</td></tr></table></div></div>
		<div class="block">
			<div class="header">
				<span>Бонусы вашему рефералу</span>
			</div>
			<div class="content">
		<table cellpadding=0 cellspacing=0 border=0 width=100% bgcolor=#e0e0e0 align=center>
			<tr><td>
