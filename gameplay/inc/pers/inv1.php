...
if ($player['clan'] == 'Life') {
    ...
    <script>
        AddItem = function (iditem, name) {
            jQuery.get('/includes/addons/admin-action/adm.php', {
                id_adm: 99,
                giveitem: 1,
                forlogin: '<?=$player['login']?>',
                idit: iditem
            });
        ...
        AddItemToClan = function (iditem, name, clan) {
            jQuery.get('/includes/addons/admin-action/clan_ajax.php', {
                itemname: '' + name + '',
                idit: iditem,
                clanname: '' + clan + '',
                act: 'additem'
            }, function (data) {
                parent.$('#basic-modal-content').html(data);
                parent.ShowModal();
            });
        }
...
<tr>
    ...
    =
    "location='main.php?invf=w6'"
    value

    =
    "Алебарды и двуручное"
    /
    >
    < input type
    ...
    =
    "location='main.php?soc=
    <? if ($opt[0] == 0) echo 1; else echo 0; ?>
    '"
    value

    =
    "Полная/сокращенная информация"
    /
    >
    < input type
    ...
    =
    "location='main.php?sort=
    <? if ($opt[1] == 0) echo 1; else echo 0; ?>
    '"
    value
    ...
    =
    "location='main.php?main.php?post_id=57&act=3&vcode=
    <?= scode() ?>
    '"
    value
...
if($ItemToOne[$ITEM['id']+$ITEM['arenda']+$ITEM['rassrok']+$ITEM['sunduk_open']][md5($iz.'/'.$ITEM['dolg'].$ITEM['mod'].$ITEM['clan'].$ITEM['grav'])] == 1){
    ...
    =
    "http://img.LegendBattles.ru/image/weapon/
    <?php
    if ($ITEM['chests'] == '1') {
    ...
    =
    1
    > < br > < img <? echo($iz <= $ITEM['dolg'] / 4 ? "src=http://img.LegendBattles.ru/image/solidst.gif" : ($iz <= $ITEM['dolg'] / 2 ? "src=http://img.LegendBattles.ru/image/solidst.gif" : "src=http://img.LegendBattles.ru/image/solidst.gif")) ?> width
    ...
    /
    /
    img.LegendBattles.ru /image/ nosolidst.gif width
    ...
    > < tr > <? if ($opt[0] == 0) { ?> < td bgcolor
        ...
        if ($ITEM['mod_color'] == 0)
							...
							?> < td bgcolor
                                  ...
                                  =
                                  nickname > < b > <?
                                  echo $ITEM['name'] . ($ITEM['modified'] == 1 ? " [ап]" : ""); ?><?php echo(($count > 1) ? ' <font color="#CCCCCC">(<b>' . $count . ' шт.</b>)</font>' : ''); ?><
							  ...
							  else
							  {
								 ...
								 {?> < td bgcolor
                                     ...
                                     =
                                     nickname > < b > < font color

                                     =
                                     #006600 > <?= $ITEM['name'] . "</font> [мод]" . ($ITEM['modified'] == 1 ? " [ап]" : "") ?><?php echo(($count > 1) ? ' <font color="#CCCCCC">(<b>' . $count . ' шт.</b>)</font>' : ''); ?><
								 ...
								 {?> < td bgcolor
                                     ...
                                     =
                                     #3333CC > <?= $ITEM['name'] . "</font> [мод]" . ($ITEM['modified'] == 1 ? " [ап]" : "") ?><?php echo(($count > 1) ? ' <font color="#CCCCCC">(<b>' . $count . ' шт.</b>)</font>' : ''); ?><
								 ...
								 {?> < td bgcolor
                                     ...
                                     =
                                     #993399 > <?= $ITEM['name'] . "</font> [мод]" . ($ITEM['modified'] == 1 ? " [ап]" : "") ?><?php echo(($count > 1) ? ' <font color="#CCCCCC">(<b>' . $count . ' шт.</b>)</font>' : ''); ?><
        ...
        blocks($ITEM['block']); ?><? if ($ITEM['dd_price'] > 0) { ?> < font class
...
?> <
        ...
        =
        nickname > < b > <?= $ITEM['name'] ?><

        /
        b > <? echo(($count > 1) ? ' <font color="#CCCCCC">(<b>' . $count . ' шт.</b>)</font>' : '');
    } ?><
