...
$act = $wsuid ?? varcheck($_POST['act']) ?? varcheck($_GET['act']) ?? '';
if ($act == 1) {
    ...
    if ($ITEM != '') {
        ...
        mysqli_query($GLOBALS['db_link'], "UPDATE user SET izym=izym+" . $ITEM['sellprice'] . " WHERE id='" . intval($uid) . "' LIMIT 1;");
        mysqli_query($GLOBALS['db_link'], "UPDATE user SET izym=izym-" . $ITEM['sellprice'] . " WHERE id='" . $player['id'] . "' LIMIT 1;");
    ...
    $ms = "top.frames['chmain'].add_msg('<font class=chattime>&nbsp;" . date("H:i:s") . "&nbsp;</font> Персонаж <b>$player[login]</b> отказался от покупки!</b></font><BR>'+'');" . $GLOBALS['redirect'];
    chmsg($ms, $login);
} else if ($act == 3) {
    ...
    if (mysqli_num_rows($ITEM) > 0) {
        ...
        mysqli_query($GLOBALS['db_link'], "UPDATE `user` SET `izym`=`izym`+'" . ($ITEM['sellprice'] * $col) . "' WHERE `id`='" . intval($uid) . "' LIMIT 1;");
        mysqli_query($GLOBALS['db_link'], "UPDATE `user` SET `izym`=`izym`-'" . ($ITEM['sellprice'] * $col) . "' WHERE `id`='" . $player['id'] . "' LIMIT 1;");
    ...
    if ($IT != '') {
        ...
        if ($col != 10 and $col != 50) {
            if ($player['izym'] >= $IT['price']) {
                if ($IT['srok'] > 0) {
                    $srok2 = 86400 * $IT['srok'];
                    $srok = time() + $srok2;
                    mysqli_query($GLOBALS['db_link'], 'INSERT INTO invent (protype,pl_id,dolg,price,arenda,srok) VALUES (' . AP . $IT['id'] . AP . ',' . AP . $player['id'] . AP . ',' . AP . $IT['dolg'] . AP . ',' . AP . $IT['price'] . AP . ',' . AP . $srok . AP . ',' . AP . $srok . AP . ');');
                } else {
                ...
                mysqli_query($GLOBALS['db_link'], 'UPDATE user SET izym=izym-' . AP . $IT['price'] . AP . ' WHERE id=' . AP . $player['id'] . AP . 'LIMIT 1;');
            ...
            if ($player['izym'] >= ($IT['price'] * $col)) {
                ...
                mysqli_query($GLOBALS['db_link'], "UPDATE `user` SET `izym`=`izym`-'" . ($IT['price'] * $col) . "' WHERE `id`='" . $player['id'] . "' LIMIT 1;");
