...
echo'</select> не дороже <input type=text size=2 name=max_nv value="'.(($_SESSION['max_nv']=='0')?'':$_SESSION['max_nv']).'" class=LogintextBox6><b>Снежинки</b> сортировка по <select name=sorttype class=zayavki><option value=1'.(($_SESSION['sorttype']=='level')?' SELECTED':'').'>уровню</option><option value=0'.(($_SESSION['sorttype']=='price')?' SELECTED':'').'>стоимости</option></select> <input type=submit value=" ok " class=lbut></font></div></form>';
?></td></tr><tr><td bgcolor=#CCCCCC width=100%><img src=/img/image/1x1.gif width=1 height=1 width=40 height=50></td></tr><!---->
<tr><td>
<div align=center>
<input input type=image src=/img/image/gameplay/shop/helm.gif onClick="location='?weapon_category=w23'" title="Шлемы" width=40 height=50><input type=image src=/img/image/gameplay/shop/armor_hard.gif onClick="location='?weapon_category=w19'" title="Доспехи" width=40 height=50>
<input type=image src=/img/image/gameplay/invent/8.gif onClick="location='?weapon_category=w61'" title="Приманки"  border="0">
<input type=image src=/img/image/gameplay/invent/1.gif onClick="location='?weapon_category=w66'" title="Алхимия"  border="0">
<input type=image src=/img/image/gameplay/invent/cat/21.gif onClick="location='?weapon_category=w0'" title="Зелья"  border="0">
...
if($num>0){?>
<table cellpadding=0 cellspacing=0 border=0 width=100%><tr><td bgcolor=#e0e0e0><table cellpadding=3 cellspacing=1 border=0 width=100%><tr><td colspan=2 bgcolor=#F9f9f9><div align=center><font class=inv><b> У Вас с собой <?=$player[sneg]?> Снежинки и вещей массой: <?=$plstt[71]?> Максимальный вес: <?=$mass?></b></div></td></tr>
<?
$freemass=$plstt[71];
...
?>
<tr><td bgcolor=#f9f9f9><div align=center><img src=/img/image/weapon/<?=$ITEM[gif]?> border=0></div></td><td width=100% bgcolor=#ffffff valign=top><table cellpadding=0 cellspacing=0 border=0 width=100%><tr><td bgcolor=#ffffff width=100%><font class=nickname><b><? if($player[sneg]>=$ITEM[price] AND $ITEM[kol]>0 and $m!=0){?><input type=button class=invbut onclick="location='main.php?post_id=116&wsuid=<?=$ITEM[id]?>&vcode=<?=scod()?>'" value="купить"> <? }if($player['access']=='admin'){
