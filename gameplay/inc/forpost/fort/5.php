...
if ($Fort['storm_stamp'] - time() < 3600 && time() < $Fort['storm_stamp']) {
    ...
    <h3 style="padding: 20 0 0 0px">Принимаются заявки на участие в штурме форта. Вы можете участвовать на
        стороне <?php echo(($Fort['clan'] == $player['clan_id']) ? 'защитников' : 'атакующих'); ?>.</h3>
    <form method="post"><input type="hidden" name="cat" value="5"><input type="submit" name="storm_process"
                                                                         value="Принять участие в штурме" class="lbut"
                                                                         onclick="return confirm('Участие на стороне атакующих стоит 100 LR. Для защитников участие бесплатно.')">
    </form>
