...
if($_GET['identify']){
	...
	if($itemcount>0){
		if($player['nv']>=($itemcount*100)){
			...
			$itnames = array();$itemsarr=array();$b=0;
			...
			for($i=0;$i<$itemcount;$i++){
				...
				if($row['id']){
					if(rand(1,100)<71 or $itemcount==1){
						...
						$itnames[] = $row['name'];
					}else{echo "<div align=center><b><font class=proce>При попытке распознания ресурс был разрушен.</font></b></div>";}
				}
			}
			if($insert){
		...
		}else{echo "<div align=center><b><font class=proce>Недостаточно денег.</font></b></div>";}
	}else{echo "<div align=center><b><font class=proce>Не найден ресурс для распознавания.</font></b></div>";}
}
