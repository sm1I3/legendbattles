...
if($_GET['buy']==1 and intval($_POST['rid'])!=""){include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/craft_sys/custom_buy".".php");}
if($_GET['create']==1 and intval($_POST['rid'])!=""){include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/craft_sys/custom_create".".php");}
$player = player();	
$pt=allparam($player);
if(intval($_GET['rec'])){
...
<table border=0 cellpadding=4 cellspacing=1 bordercolor=#e0e0e0 align=center class="smallhead" width=100%>
	...
	while ($row = mysqli_fetch_assoc($recipes)){
			...
			if($player['custom_rec']=="0"){$alhim_rec="||||";}else{$alhim_rec=explode("|",$player['custom_rec']);}
			echo'</td></tr>
