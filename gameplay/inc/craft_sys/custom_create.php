<?
	...
	if(mysqli_num_rows($chrecipe)>0){
		...
		if($player['custom_rec']=="0"){$alhim_rec = "0";}
		else{$alhim_rec = explode("|",$player['custom_rec']);}	
		if(in_array($chrecipe['id'],$alhim_rec)){
			...
			if($IT!='' and $err=="0"){
				...
				$message=5;include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/craft_sys/custom_messages".".php");
			}
			else if($err=="1"){$message=6;include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/craft_sys/custom_messages".".php");}
			else if($IT==''){$message=7;include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/craft_sys/custom_messages".".php");}
		}
		else{
			$message=8;
