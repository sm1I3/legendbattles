<?
	...
	if(mysqli_num_rows($chrecipe)>0){
		...
		if($player['custom_rec']=="0"){$koldyn_rec = "0";}
		else{$koldyn_rec = explode("|",$player['custom_rec']);}	
		if(in_array($chrecipe['id'],$koldyn_rec)){
			$reg = explode("|",$chrecipe['reagents']);
			$regmiss="";
			$grassit = mysqli_query($GLOBALS['db_link'],"SELECT * FROM `items` WHERE `type`='w66' OR `type`='w67' OR `type`='w68' OR `type`='w69' OR `type`='w70' OR `type`='w71' OR `type`='w100' AND `slot`='0';");
			...
			if($IT!='' and $err=="0"){
				...
				$insert="";
				for($i=0;$i<$chrecipe['col'];$i++){$insert.="('".$IT['id']."','".$player['id']."','".$dolg."','".$IT['price']."'),";}
				...
				$message=5;include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/craft_sys/custom_messages2".".php");
			}
			else if($err=="1"){$message=6;include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/craft_sys/custom_messages2".".php");}
			else if($IT==''){$message=7;include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/craft_sys/custom_messages2".".php");}
		}
		else{
			$message=8;
