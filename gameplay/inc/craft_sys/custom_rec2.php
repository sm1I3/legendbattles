...
if($_GET['buy']==1 and intval($_POST['rid'])!=""){include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/craft_sys/custom_buy2".".php");}
if($_GET['create']==1 and intval($_POST['rid'])!=""){include ($_SERVER["DOCUMENT_ROOT"]."/gameplay/inc/craft_sys/custom_create2".".php");}
$player = player();	
$pt=allparam($player);
if(intval($_GET['rec'])){
	switch(intval($_GET['rec'])){
		case 1: $_SESSION['user']['hospi_rec']=1; break;
		case 2: $_SESSION['user']['hospi_rec']=2; break;
	}
}
if($_SESSION['user']['hospi_rec']==''){$_SESSION['user']['hospi_rec']=1;}
echo'
...
<table border=0 cellpadding=4 cellspacing=1 bordercolor=#e0e0e0 align=center class="smallhead" width=100%>
	...
	switch($_SESSION['user']['hospi_rec']){
		case 1: $str="WHERE `items`.`type`='w70'"; break;
		case 2: $str="WHERE `items`.`type`='w71' OR `items`.`type`='w100'"; break;
	}
	...
	while ($row = mysqli_fetch_assoc($recipes)){
			...
			if($player['custom_rec']=="0"){$koldyn_rec="||||";}else{$koldyn_rec=explode("|",$player['custom_rec']);}
			echo'</td></tr>
