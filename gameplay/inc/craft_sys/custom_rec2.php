...
if ($_GET['buy'] == 1 and intval($_POST['rid']) != "") {
    include($_SERVER["DOCUMENT_ROOT"] . "/gameplay/inc/craft_sys/custom_buy2" . ".php");
}
if ($_GET['create'] == 1 and intval($_POST['rid']) != "") {
    include($_SERVER["DOCUMENT_ROOT"] . "/gameplay/inc/craft_sys/custom_create2" . ".php");
}
$player = player();
$pt = allparam($player);
if (intval($_GET['rec'])) {
...
while ($row = mysqli_fetch_assoc($recipes)) {
    ...
    if ($player['custom_rec'] == "0") {
        ...
        $koldyn_rec = explode("|", $player['custom_rec']);
    }
    echo '</td></tr>
