...
<?		}
		...
		<b><i>&nbsp;<?=$player['semija'] ?>     <? if($player['semija'] !='не замужем'){?><? if($player['semija'] !='не женат'){?><img src=img/image/rings.gif ><?}?><?}?></i></b>
	</div>
<?}
...
?>
      <div class="module comp">
		...
		<div class="content">
			...
			<input type=button class=invbut onClick="javascript: location='main.php?post_id=57&act=3&vcode=<?=scode()?>'" value="Снять все вещи">
			<div id="complect"></div>
		</div>
	</div>
<?
...
<?php
$trw=affect($player[affect],3,true);foreach ($trw as $key => $val){$plst[$key]+=$val;}
...
foreach($st as $key=>$val){if($val<=0)$st[$key]=0;}
...
<? for ($i=0;$i<=4;$i++) {?>
		<div>
			<img src="img/images/stats/st<?=$i?>.jpg" /> <?=$stats[$i]?>: <span class="cnt" id="st<?=$i?>"><?=$plstt[30+$i]?></span>
<?	if($plst[30+$i]){?>
...
<?}?>
<? if($player['free_stat']>0){?>
		<tr><td colspan=5 class=freemain><div align=center><a href="javascript: SaveStats();">Сохранить</a></div></td></tr>
		<?
...
<div class="module modif">
	...
	<div class="content">
		<div>
			Утомление : <span class="cnt"><? if($player['ustal']<time()){echo 0;}else{echo round(($player['ustal']-time())/(150/($plstt[58]/200+1)));}?>%</span>
		</div>
		<div>
...
<? if($player[vzlomshik_nav]>0){?>
		<div>
			Взломщик : <span class="cnt" onmouseover="tooltip(this,'Взломанных сундуков: <b><?php echo $player[vzlomshik_exp];?> </b>')" onmouseout="hide_info(this)"><?=$player[vzlomshik_nav]?></span>
		</div>
<?}?>
