...
if ($_GET['grav'] and $_POST['item']) {
    if ($_POST['grav']) {
        ...
        $err = 0;
        if (testchr($name) == 1) {
            $message = "<font style=\"color:red;\" class=nickname2><b>гравировка содержит недопустимые символы.</b></font>";
            $err = 1;
        } else if ($name == '') {
            $message = "<font style=\"color:red;\" class=nickname2><b>гравировка не может состоять только из пробелов.</b></font>";
            $err = 1;
        } else if ($err == 0) {
            ...
            if (mysqli_num_rows($dditems) > 0) {
                if ($player['baks'] >= 5) {
                    if (mysqli_query($GLOBALS['db_link'], "UPDATE `invent` SET `grav`='" . $name . "' WHERE `id_item`='" . intval($_POST['item']) . "' AND `pl_id`='" . $player['id'] . "' LIMIT 1;") and mysqli_query($GLOBALS['db_link'], "UPDATE `user` SET `baks`=`baks`-'5' WHERE `id`='" . $player['id'] . "' LIMIT 1;")) {
                        ...
                        $message = "<font style=\"color:red;\" class=nickname2><b>Гравировка не выполнена.</b></font>";
                    }
                } else {
                    $message = "<font style=\"color:red;\" class=nickname2><b>Гравировка не выполнена. Недостаточно денег.</b></font>";
                }
            } else {
                $message = "<font style=\"color:red;\" class=nickname2><b>Гравировка не выполнена. Вещь не найдена.</b></font>";
            }
        }
    }
