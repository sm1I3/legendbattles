...
if($act==1){
	...
	if($ITEM!=''){
		...
		mysqli_query($GLOBALS['db_link'],"UPDATE user SET reputdr=reputdr+".$ITEM['sellprice']." WHERE id='".intval($uid)."' LIMIT 1;");
		mysqli_query($GLOBALS['db_link'],"UPDATE user SET reputdr=reputdr-".$ITEM['sellprice']." WHERE id='".$player['id']."' LIMIT 1;");
		...
		$plmsg="top.frames['chmain'].add_msg('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> Вы удачно купили <b>".$ITEM[name]."</b> за <b>".$price."</b> !</b></font><BR>'+'');$redirect";
	...
	if(mysqli_num_rows($ITEM)>0){
		...
		mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `reputdr`=`reputdr`+'".($ITEM['sellprice']*$col)."' WHERE `id`='".intval($uid)."' LIMIT 1;");
		mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `reputdr`=`reputdr`-'".($ITEM['sellprice']*$col)."' WHERE `id`='".$player['id']."' LIMIT 1;");
	...
	if($IT!=''){
			...
			if($col!=10 and $col!=50){
				if($player['reputdr']>=$IT['price']){
					...
					mysqli_query($GLOBALS['db_link'],'UPDATE user SET reputdr=reputdr-'.AP.$IT[price].AP.' WHERE id='.AP.$player[id].AP.'LIMIT 1;');
			...
			elseif(($col==10 or $col==50) and $IT['kol']>=$col){
				if($player['reputdr']>=($IT['price']*$col)){
					$insert="";
					for($i=0;$i<$col;$i++){
					...
					mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `reputdr`=`reputdr`-'".($IT['price']*$col)."' WHERE `id`='".$player['id']."' LIMIT 1;");
		...
		WHERE kol>0 AND items.dd_price>0 AND items.id=$wsuid LIMIT 1;"));
			if($IT!=''){
				if($player[baks]>=$IT[dd_price]){
					...
					$msg="<b><font class=proce>Вы удачно купили:<br><font class=proceg> ".$IT['name']." </font><br></font></b>"; log_write("buy",$IT[name],$IT[dd_price],"market");
				}
				else{$msg="<b><font class=proce>Нехватает денег!</font></b>"; }
			}
	}
