<?
	...
	if($pl_st[101]!=0 and $tg[hp]>0 and $immunes[0]==1){
		if($tg['invisible']<time()){
			$log.=",$logtg,\" не получил$tsex[1] урон от огн€, потому что имеет ".$immunes_arr[0]." [$tg[hp]/$tg[hp_all]].\"";
		}else{
			$log.=",$logtg,\" не получил$tsex[1] урон от огн€, потому что имеет ".$immunes_arr[0]." [???/???].\"";
		}
	}	
	elseif($pl_st[101]!=0 and $tg[hp]>0){
		...
		$exp[0]+=$dps;
		if($tg['invisible']<time()){
			$log.=",$logtg,\" получил$tsex[1] урон от огн€ на <B><font color=#$clr>-$dps</font></B> [$tg[hp]/$tg[hp_all]].\"";
		}else{
			$log.=",$logtg,\" получил$tsex[1] урон от огн€ на <B><font color=#$clr>-$dps</font></B> [???/???].\"";
		}
	}
