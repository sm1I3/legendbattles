...
switch($post_id){
		...
		if($dditem['id_item']){
			...
			switch($act){
				case 1:
						...
						else{$msg="<b><font class=proce>Вы не можете продавать клановые вещи!</font></b>";}
				...
				case 2:
					...
					else{$msg="<b><font class=proce>Вы не состоите в клане или эта вещь уже в казне!</font></b>";}
				break;
				case 3:
					settype($prot, "int");
				...
				if($pza){
					...
					if(testarena($bid[1])!=0){
						...
						if($ok>0){
							...
							if($_SESSION[user]['ft']==1){sumbat($bid[1],"parent.frames['chmain'].add_msg('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b><font color=#cc0000>Внимание!</font></b> &nbsp;<b>".$_SESSION[user][login]."</b> принял вашу заявку! </font><BR>'+'');$redirect",1);}
	...
	case 20:
			...
			if($pid!='' and $prnick!=$player['login']){
				...
				if($pl[id]!='')	{
					if(!$pranon){$message="$prtext ($player[login])";}else{$message="$prtext";}
						...
						$chatms="parent.frames['chmain'].add_msg('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b><font color=#CC0000>Внимание!</font></b>&nbsp;Вам сделан подарок от <b>(".($pranon?'анонимно':$player['login']).")</b>!&nbsp;$chprtext</font></font><BR>'+'');";chmsg($chatms,$prnick);
				}
				else{$msg="<br><div align=center><b><font class=proce>Персонаж $prnick не найден!</font></b></div>";}
			}
			else{$msg="<br><div align=center><b><font class=proce>Нельзя дарить подарки себе!</font></b></div>";}
		...
		}else{$msg="<br><div align=center><b><font class=proce>Нехватает денег!</font></b></div>";}
	break;
	case 21:
		$cut="...";
		$prtext=chars(str_replace ("%", "",$prtext));
				...
				if($pid!='' and $prnick!=$player['login']){
					...
					if($pl[id]!='')	{
						if(!$pranon){$message="$prtext ($player[login])";}else{$message="$prtext";}
							...
							mysqli_query($GLOBALS['db_link'],'INSERT INTO podarki(id,podarok,srok,message)VALUES('.AP.$pl[id].AP.', '.AP.$pid.AP.', '.AP.(time()+$pod[srok]).AP.', '.AP.$message.AP.');');
							$chprtext="Надпись на подарке:&nbsp;<font color=#CC0000><b> $prtext</b></font>";
							$chatms="parent.frames['chmain'].add_msg('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b><font color=#CC0000>Внимание!</font></b>&nbsp;Вам сделан подарок от <b>$player[login]</b>!&nbsp;$chprtext</font></font><BR>'+'');";chmsg($chatms,$prnick);
					}
				else{$msg="<br><div align=center><b><font class=proce>Персонаж $prnick не найден!</font></b></div>";}
				}
			else{$msg="<br><div align=center><b><font class=proce>Нельзя дарить подарки себе!</font></b></div>";}
	...
	break;
	case 22: $sum = intval($_POST['gold']) * 10000 + intval($_POST['silver']) * 100 + intval($_POST['bronze']); $fornickname=trim($fornickname);if($fornickname!='' and intval($sum)>0){if($player['login']!=$fornickname){$ret=transfer($transferuid,$fornickname,$player[loc],$transfernametxt,$player[login],intval($sum),$ttext);$msg=$ret[msg];}else{$msg="<b><font class=proce>Вы не можете передавать себе!</font></b><br>";}}break;
	case 23: $fornickname=trim($fornickname);if($player['login']!=$fornickname){$ret=gift($presentuid,$fornickname,$player[loc],$presentnametxt,$player[login],$presentnv);$msg=$ret[msg];}else{$msg="<b><font class=proce>Вы не можете подарить себе!</font></b><br>";}break;
	...
	case 25: 
		...
		if($sum>=1 and $player['baks']>=round($sum)){
			...
			else{$msg="<font class=proceg><b>Ошибка при обмене.</b></font>";}
		}
		else{$msg="<font class=proceg><b>Ошибка при обмене (неверная сумма).</b></font>";}
	break;
	case 26: 
	...
	case 34:
		if($bank_act==2){
			if($player[nv]<5){$msg="<div align=center><b><font class=proce>Нехватает денег.</font></b></div>";}
			else{
				if($bpsw!='' and $bpsw==$bpsw1){
					mysqli_query($GLOBALS['db_link'],'INSERT INTO bank (id,pass,email)VALUES('.AP.$player[id].AP.', '.AP.$bpsw.AP.', '.AP.$bmail.AP.');');$msg="<div align=center><b><font class=proce>Счет открыт.</font></b></div>";mysqli_query($GLOBALS['db_link'],"UPDATE user SET nv=nv-5 WHERE `id` = '".$player['id']."'");}
				else{$msg="<div align=center><b><font class=proce>Ошибка в пароле!</font></b></div>";}
			}
		}
		if($bank_act==3){
			...
			else{$msg="<div align=center><b><font class=proce>Неверный пароль.</font></b></div>";}
		}
	break;
		...
		if(mysqli_query($GLOBALS['db_link'],"DELETE FROM `podarki` WHERE `podarok`='".$wn."' and `id`='".$player['id']."' LIMIT 1;")){
				...
				foreach($item as $val){
					if($val!=''){
							...
							if(mysqli_query($GLOBALS['db_link'],"INSERT INTO invent (`protype` ,`pl_id` ,`dolg` ,`price` ,`gift`,`gift_from`,`death`) VALUES ('".$itemsql['id']."','".$player['id']."','".$dolg."','".$itemsql['price']."','1','legendbattles.ru',".$death.");")){
								$ms="parent.frames['chmain'].add_msg('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b><font color=#CC0000>Внимание!</font></b></font>&nbsp;Вы получили подарок: <b>".$itemsql['name']."</b>.</font><BR>'+'');";chmsg($ms,$player['login']);
							}
					}
				}
			}
			if($items['dlr']>0){
				if(mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `dd`=`dd`+'".$items['dlr']."' WHERE `id`='".$player['id']."' LIMIT 1;")){
					$ms="parent.frames['chmain'].add_msg('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b><font color=#CC0000>Внимание!</font></b></font>&nbsp;Вы получили подарок: <b>".$items['dlr']." DLR</b>.</font><BR>'+'');";chmsg($ms,$player['login']);
				}
			}
			if($items['lr']>0){
				if(mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `nv`=`nv`+'".$items['lr']."' WHERE `id`='".$player['id']."' LIMIT 1;")){
					$ms="parent.frames['chmain'].add_msg('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b><font color=#CC0000>Внимание!</font></b></font>&nbsp;Вы получили подарок: <b>".$items['lr']." LR</b>.</font><BR>'+'');";chmsg($ms,$player['login']);
				}
			}
...
//				mysqli_query($GLOBALS['db_link'],'UPDATE user SET exp='.AP.implode("|", $exp).AP.', dmg = default, battle='.AP.'0'.AP.', fight='.AP.'0'.AP.' WHERE id='.AP.$player['id'].AP.';');
				...
				switch(intval($_GET['act'])){
					case 1:
						...
						if($player['baks']>=round($IT['dd_price']*3/50+3)){
							...
							$msg="<b><font class=proce>Вы удачно арендовали \"$IT[name]\"!</font></b>"; log_write("buy",$IT[name],round($IT[dd_price]*3/50+3),"market");
							$typetolog .= '@14';  
							$abouttolog .= '@<b>'.$IT['name'].'</b> . По цене: <b>'.round($IT[dd_price]*3/50+3).'</b> Изумруд. [10 дней]';
						}
						else{$msg="<b><font class=proce>Нехватает денег!</font></b>"; }
					break;
					case 2:
						...
						if($player['baks']>=round($IT['dd_price']*3/35+4)){
							...
							$msg="<b><font class=proce>Вы удачно арендовали \"$IT[name]\"!</font></b>"; log_write("buy",$IT[name],round($IT[dd_price]*3/35+4),"market");
							$typetolog .= '@14';  
					...
					case 3:
						...
						if($player['baks']>=round($IT['dd_price']*3/20+5)){
							...
							$msg="<b><font class=proce>Вы удачно арендовали \"$IT[name]\"!</font></b>"; log_write("buy",$IT[name],round($IT[dd_price]*3/20+5),"market");
							$typetolog .= '@14';  
					...
					case 4:
						...
						if($player['baks']>=round($IT['dd_price']*3/110+1)){
							...
							$msg="<b><font class=proce>Вы удачно арендовали \"$IT[name]\"!</font></b>"; log_write("buy",$IT[name],round($IT[dd_price]*3/110+1),"market");
							$typetolog .= '@14';  
					...
					case 5:
						...
						if($player['baks']>=round($IT['dd_price']*3/80+2)){
							...
							$msg="<b><font class=proce>Вы удачно арендовали \"$IT[name]\"!</font></b>"; log_write("buy",$IT[name],round($IT[dd_price]*3/80+2),"market");
							$typetolog .= '@14';  
				...
				switch(intval($_GET['act'])){
					case 1:
						...
						if($player['baks']>=round($IT['dd_price']/2+1)){
							...
							$msg="<b><font class=proce>Вы удачно взяли в рассрочку \"$IT[name]\"!</font></b>"; log_write("buy",$IT[name],round($IT[dd_price]/2+1),"market");
							$typetolog .= '@15';  
					...
					case 2:
						...
						if($player['baks']>=round($IT['dd_price']/3+1)){
							...
							$msg="<b><font class=proce>Вы удачно взяли в рассрочку \"$IT[name]\"!</font></b>"; log_write("buy",$IT[name],round($IT[dd_price]/3+1),"market");
							$typetolog .= '@15';  
					...
					case 3:
						...
						if($player['baks']>=round($IT['dd_price']/4+1)){
							...
							$msg="<b><font class=proce>Вы удачно взяли в рассрочку \"$IT[name]\"!</font></b>"; log_write("buy",$IT[name],round($IT[dd_price]/4+1),"market");
							$typetolog .= '@15';  
	...
	case 101: 
		...
		if($fornickname!=$player['login']){
			...
			else{$msg='<b><font class=proce>Сделка невозможна! Персонаж отсутствует либо в бою!</font></b>';}
			unset($pl);
		}
	break;
	case 106:
		if($_GET['act'] and $player['present']==0 and in_array($_GET['vcode'],$_SESSION['secur']) or $player['login']=='mozg'){
			...
			$acc = explode("|",$player['premium']);
			if($acc[0]>2){
				if($acc[1]<time()){$timer = time()+(86400*14);$acc[0]=$items['account'];} # 14 дней
				else{$timer = $acc[1]+(86400*14);} # 14 дней к текущему времени аккаунта
				$premium = $acc[0]."|".$timer;
			}else{
				if($acc[1]<time()){$timer = time()+(86400*14);} # 14 дней
				else{$timer = $acc[1]+(86400*14);} # 14 дней к текущему времени аккаунта
				$premium = "3|".$timer;
			}
	...
	case 108:
		switch(intval($_GET['act'])){
			case 1:
				...
				if($timer2 < $timer){$msg="Неверный таймер переработки";}
				else{
					$pt=explode("|",$pers['st']);
		...
		switch(intval($_GET['act'])){
			...
			case 3:
				...
				if($stopbuy==0){
					...
					else{$msg="<font class=proce>Предмет не найден или указанное количество предметов отсутствует в магазине.</font>";}
				}
				...
				if($player['login'] == 'alexs' or $player['login'] == 'Администрация'){	
					...
					if($stopbuy==0){
						...
						if($IT['id']){
							...
							else{$msg="<font class=proce>Запрос не удался.</font>";}
						}
						else{$msg="<font class=proce>Предмет не найден";}
					}
					else{$msg="<font class=proce>Неизвестная ошибка.</font>";}
				}
				else{$msg="<font class=proce>Нет доступа</font>";}
			break;
			case 5:
				if($player['login'] == 'alexs' or $player['login'] == 'Администрация'){	
					$stopbuy=0;
	...
	break;
	case 111:
		if($player['login'] == 'alexs' or $player['login'] == 'Администрация'){mysqli_query($GLOBALS['db_link'],"DELETE FROM `market` WHERE `id`='".intval($_GET['wsuid'])."' AND `market`='".intval($_GET['market'])."' LIMIT 1;");$msg="Вещь удалена.";}
	break;
	case 112:
...
//						echo $item[(count($item)-2)];
						...
						if(mysqli_query($GLOBALS['db_link'],"INSERT INTO invent (`protype` ,`pl_id` ,`dolg` ,`price`) VALUES ('".$itemsql['id']."','".$player['id']."','".$dolg."','".$itemsql['price']."');")){
							...
							if($items['lr']>0) {echo"<script>top.frames['chmain'].add_msg_system('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b>Вы обнаружили внутри: <b>".lr($items['lr'])."</b>.</font>'+'');</script>";}
							if($items['reput']>0) {echo"<script>top.frames['chmain'].add_msg_system('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b>Вы получили репутацию <b>".$items['reput']."</b>.</font><BR>'+'');</script>";}
							if($items['RepsPodvod']>0) {echo"<script>top.frames['chmain'].add_msg_system('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b>Вы получили репутацию1 <b>".$items['RepsPodvod']."</b>.</font><BR>'+'');</script>";}
...
//						echo $item[(count($item)-2)];
            mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `nv`=`nv`+'".$items['lr']."' WHERE `id`='".$player['id']."'");
						...
						if(mysqli_query($GLOBALS['db_link'],"INSERT INTO invent (`protype` ,`pl_id` ,`dolg` ,`price`) VALUES ('".$itemsql['id']."','".$player['id']."','".$dolg."','".$itemsql['price']."');")){
							...
							if($items['lr']>0) {echo"<script>top.frames['chmain'].add_msg('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b>Вы обнаружили внутри: деньги <b>".lr($items['lr'])."</b>.</font>'+'');</script>";}
							if($items['reput']>0) {echo"<script>top.frames['chmain'].add_msg('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b>Вы получили репутацию <b>".$items['reput']."</b>.</font><BR>'+'');</script>";}
							if($items['RepsPodvod']>0) {echo"<script>top.frames['chmain'].add_msg('<font class=chattime>&nbsp;".date("H:i:s")."&nbsp;</font> <font color=000000><b>Вы получили репутацию1 <b>".$items['RepsPodvod']."</b>.</font><BR>'+'');</script>";}
