...
if ($post_id == 2) {
    ...
    if ($ids != '' and $slots != '' and $i == 0 and $cname != '') {
        mysqli_query($GLOBALS['db_link'], 'INSERT INTO pcompl(uid,name,ids,slots) VALUES(' . AP . $player[id] . AP . ',' . AP . $cname . AP . ',' . AP . $ids . AP . ',' . AP . $slots . AP . ')');
        $msg = '<b><font class=nickname><font color=#cc0000>Комплект "' . $cname . '" сохранен!</font></font></b><br>';
    }
    if ($ids != '' and $slots != '' and $i != 0 and $cname != '') {
        mysqli_query($GLOBALS['db_link'], 'UPDATE pcompl SET ids=' . AP . $ids . AP . ', slots=' . AP . $slots . AP . ' WHERE  uid=' . AP . $player[id] . AP . ' and name=' . AP . $cname . AP . ';');
        $msg = '<b><font class=nickname><font color=#cc0000>Комплект "' . $cname . '" заменен!</font></font></b><br>';
    }
}
