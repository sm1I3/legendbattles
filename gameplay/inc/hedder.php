...
if($row["id_battle"]==''){mysqli_query($GLOBALS['db_link'],"UPDATE user SET battle='0' WHERE id='".$user."'");$msg=0;}else{$msg=1;}
return $msg;
}
//end function
if($player['battle']<>0)
...
if($player['pos']!='12_9' and !$player['prison'] and $player['login']!='Администрация' and !in_array($player['loc'],$locs)){
...
$name = array('','?ети Уьмы','?ети Чвета','?ети Чумерек','?ети Таоса','їстинный Чвет','їстинна§ Уьма','Нейтральные Чумерки','Љбсолютный Таос','Љнгел');
...
<? if($_SESSION['user']['pos']==1){?><SCRIPT src="./js/compl.js"></SCRIPT>
...
<? if($_SESSION['user']['pos']<2){?><SCRIPT src="/js/slots.js?v1"></SCRIPT>
<SCRIPT src="/js/transfer.js"></SCRIPT>
<SCRIPT src="/js/effects_v03.js"></SCRIPT>
<? if($player['free_stat']>0){?><SCRIPT src="/js/addstat.js"></SCRIPT><? }?>
...
<? if($vis[1]>time()){ ?>
		<div class="hurt">
			<b>Тяжелые увечья!</b> Невозможно совершать никаких действий еще: <?=date("Hч. iм. sс.",($vis[1]-time()))?>
		</div>
<?}?>
<div class="TopBar">
		<div class="TopBar_left">
			...
			<img src='img/image/signs/<?=$img[$clan_p['clan_sclon']]?>.gif' <?=(($clan_p['clan_sclon'])?"title='".$name[$clan_p['clan_sclon']]."' ":"")?>width=15 height=12 align=absmiddle border=0>
			<img src="img/image/signs/<?=$clan_p['clan_gif']?>" width="15" height="12" align="absmiddle" title="<?=$clan_p['clan_name']?> (<?=$player['clan_d']?>)"/>
<?}?>		 <span style="font-weight: normal;"><?=$_SESSION['user']['login']?>	[<?=$player['level']?>]</span>
				</center>
				</span>
...
if($player['loc']==28){
	if($ret[0]!=0 and $dis==0 and $_SESSION['user']['pos']>1){?>
			<a href="main.php?get=3&go=<?=$ret[0]?>&vcode=<?=scode()?>" class="MovementMenu mainTooltip"></a>
<?}?>
			<a href="<?=(($_SESSION['user']['pos']>1)?'#':'main.php?get=3&go='.$player['loc'].'&vcode='.scode())?>" class="MovementMenu mainTooltip"></a>
<?	}
...
<?	}
	...
	if($player['level'] > 4){?>
			<a href="<?=(($_SESSION['user']['pos']==2 or $player['battle']!=0 or $dis==1)?'#':'main.php?get=2&go='.$ret[0].'&vcode='.scode().'" onmouseover="tooltip(this,\''.$ret[1].'\')" onmouseout="hide_info(this)')?>" class="MovementMenu mainTooltip"></a>
<?	}
}
if($_SESSION['user']['pos']<2 and $ret[4]==0){?>
			<a href="main.php?get=3&vcode=<?=scode()?>" class="MovementMenu mainTooltip"></a>
<?	}?>
			<div class="hours" style="float: right;margin-top:1px;margin-right: 121px;"><img src="img/razdor/emerald.png" width="14" height="14" title="+1 »зумруд"></div>
			<div class="lines" style="float: right;">
				<div class="dlr" style="margin-top: 8px;margin-right: 3px;">
					...
					<div class="cnt" id="dlrcnt">≈ще <?=((($player['onlineBouns']-time())>=3600)?"60:00":date("i:s",($player['onlineBouns']-time())))?></div>
...
<?}?></div>
				...
				setInterval(function() {
					...
					if (seconds || minutes) {
						document.getElementById("dlrcnt").innerHTML = '≈ще '+minutes+':'+seconds;
			...
			<ul class="MainMenu">
				<li class="CharacterMenu">
					<a href="<?=(($player['battle']!=0)?'#':'main.php?get=0&vcode='.scode())?>" id="CharacterMenu" class="mainTooltip" onmouseover="tooltip(this,'<b>ѕерсонаж</b>');" onmouseout="hide_info(this);"></a>
				</li>
				<li class="InventoryMenu">
					<a href="<?=(($_SESSION['user']['pos']==1 or $player['battle']!=0)?'#':'main.php?get=1&vcode='.scode().(($_SESSION['user']['pos'] == 0 and $player['NewGameSteps'] == 0)?'&new-steps=2':''))?>" id="InventoryMenu" class="mainTooltip" onmouseover="tooltip(this,'<b>»нвентарь</b>');" onmouseout="hide_info(this);"></a>
				</li>
				<li class="FightingMenu">
					<a href="#" id="FightingMenu" class="mainTooltip" onmouseover="tooltip(this,'<b>ѕоединки</b>');" onmouseout="hide_info(this);"></a>
				</li>
				<li class="ClanMenu">
					<a <?if($player['clan_id']!='none'){?> href="core2.php?useaction=clan-action" <?}?> id="ClanMenu" class="mainTooltip" onmouseover="tooltip(this,'<b> лан</b>');" onmouseout="hide_info(this);" ></a>
				</li>
				<li class="InfoMenu">
					<a href="javascript:QActive('<?=scode()?>');" id="InfoMenu" class="mainTooltip" onmouseover="tooltip(this,'<b> весты</b>');" onmouseout="hide_info(this);"></a>
				</li>
			</ul>
		</div>
		</div>
