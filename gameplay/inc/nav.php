...
if ($freeskills >= 0 and $freeskillsmir >= 0 and $fail == 0) {
    if ($f[30] > 0 or $f[33] > 0) {
        $hps = (10000 / ($f[30] / 100 + 1));
        $mps = (10000 / ($f[33] / 100 + 1));
        mysqli_query($GLOBALS['db_link'], 'UPDATE user SET hps=' . AP . $hps . AP . ', mps=' . AP . $mps . AP . ' WHERE id=' . AP . $player['id'] . AP . 'LIMIT 1;');
    }
    $um = implode("|", $f);
    mysqli_query($GLOBALS['db_link'], 'UPDATE user SET umen=' . AP . $um . AP . ', fr_bum=' . AP . $freeskills . AP . ', fr_mum=' . AP . $freeskillsmir . AP . ' WHERE login=' . AP . $_SESSION['user']['login'] . AP . 'LIMIT 1;');
    calchp();
    calcstat($player['id']);
}
