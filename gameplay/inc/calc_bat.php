...
if($tg[bl]>0){$ms[2]=" щитом";$ms[3]=" щит";}else{$ms[3]=" блок";}
$cblock=($pl_st[30]-$tg_st[30])/2+(($player[level]-$tg[level])*5)+rand(0,50)+($player[bl]/2); 
if($cblock<5){$cblock=5;}else if($cblock>95){$cblock=95;} 
if($player['sex']=='female'){$psex=array(0=> 1,"a","ась");}else{$psex=array(0=> 0,"","ся");}
if($tg['sex']=='female'){$tsex=array(0=> 1,"a","ась");}else{$tsex=array(0=> 0,"","ся");}
$bl=block($block);
$arr=explode("@",$inu);
...
switch ($bl[1]) {
    case 4: return array(0);break; //голова
