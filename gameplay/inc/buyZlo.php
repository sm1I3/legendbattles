...
if($act==1){
	...
	if($ITEM!=''){
		...
		mysqli_query($GLOBALS['db_link'],"UPDATE user SET Zlo=Zlo+".$ITEM['sellprice']." WHERE id='".intval($uid)."' LIMIT 1;");
		mysqli_query($GLOBALS['db_link'],"UPDATE user SET Zlo=Zlo-".$ITEM['sellprice']." WHERE id='".$player['id']."' LIMIT 1;");
	...
	if(mysqli_num_rows($ITEM)>0){
		...
		mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `Zlo`=`Zlo`+'".($ITEM['sellprice']*$col)."' WHERE `id`='".intval($uid)."' LIMIT 1;");
		mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `Zlo`=`Zlo`-'".($ITEM['sellprice']*$col)."' WHERE `id`='".$player['id']."' LIMIT 1;");
	...
	if($IT!=''){
			...
			if($col!=10 and $col!=50){
				if($player['Zlo']>=$IT['price']){
					...
					mysqli_query($GLOBALS['db_link'],'UPDATE user SET Zlo=Zlo-'.AP.$IT[price].AP.' WHERE id='.AP.$player[id].AP.'LIMIT 1;');
			...
			elseif(($col==10 or $col==50) and $IT['kol']>=$col){
				if($player['Zlo']>=($IT['price']*$col)){
					...
					mysqli_query($GLOBALS['db_link'],"UPDATE `user` SET `Zlo`=`Zlo`-'".($IT['price']*$col)."' WHERE `id`='".$player['id']."' LIMIT 1;");
