...
$sql=mysqli_query($GLOBALS['db_link'],"SELECT * FROM logs WHERE bid='$player[battle]' ORDER BY `id` DESC LIMIT 0,10;");while ($log = mysqli_fetch_assoc($sql)) {if($log['list']==0){$logs.=$log['log'];}else{$list=$log['log'];}}
...
if($vis==3){$bat=0;$ty=2;$l=1;}else{
if($player['hp']>0){
if(mysqli_num_rows($sql)>0){$bat=1;$ty=0;
...
$en_st=explode("|",$en[st]);
for($i=5;$i<=36;$i++){if($en_st[$i]=='')$en_st[$i]=0;}
if(!isset($en['id'])){$bat=0;$ty=3;$_SESSION['user']['wait']=1;}else{$_SESSION['user']['wait']=0;
...
$bat=0;$ty=5;$end=scode();}}else{$bat=0;$ty=4;$end='';}}
...
?>
var slots_en = ["<?=$en['obraz'];?>","<? if($en['type']==1){slotwiev($en[id],0);}else{if($en[addon]==99){echo'1';slotwiev($player[id],0);}else {botslot($eninv[id],0);}}?>"];
...
if($l==1) {
	echo "var list = [";
	if(is_file("/var/www/legen451/data/www/battles/stats/".$player['battle'].".txt")){
		echo Show_Stat($player['battle']);
	}
	echo"];\n";
?> var fexp = ["<?=$player['dmg']?>","15","<?=$player['id']?>","<?=scod()?>","<?=$randomize?>","",<? if($player[wait]>time()){print $player[wait]-time();}else{print 0;}?>,0]; <?
}else{?>
