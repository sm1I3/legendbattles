...
function use_b($id, $pl, $sex)
{
    ...
    if ($it['num_a']) {
        switch ($it[num_a]) {
            case 32:
                $pl[hp] += $it[effect];
                $msg[msg] = "[7,\"Зелье Восстановления $it[effect] HP\",$sex[0]],\".\"";
                break;
            case 33:
                $pl[mp] += $it[effect];
                $msg[msg] = "[7,\" Зелье Восстановления $it[effect] MP\",$sex[0]],\".\"";
                break;
            case 320:
                $ma = ($pl[hp_all]) / 2;
                $maneed = round(($pl[hp_all] - $pl[hp]) / 2);
                if ($maneed < $ma) {
                    $ma = $maneed;
                }
                if ($ma > $pl[mp]) {
                ...
                $pl[hp] += $ma * 2;
                $pl[mp] -= $ma;
                $msg[msg] = "[7,\"Заклинание Восстановления HP\",$sex[0]],\" и\",[2," . ($ma * 2) . ",\"HP\",$sex[0]]";
                break;
        }
    } else {
