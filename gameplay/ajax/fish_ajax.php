...
if(new_array($pers)=='ok'){$pers['sign']=$sk;$pers['fish_skill']+=200;}
if($pers['id']){
    $pt=explode("|",$pers['st']);
    ...
    case 1:
        ...
        if($pers['login']=='Администрация' or $pers['login']=='m0ne'){$timer=20;}
        ...
        if(mysqli_num_rows($grasssql)<1){$error="Тут рыбы нет, или уровень вашего навыка недостаточен для ловли рыбы в этом месте.";}
        else{
            $error="";
            $grassrow="";
            ...
            if($allgrass==""){$error="Тут рыбы нет, или уровень вашего навыка недостаточен для ловли рыбы в этом месте.";}		
        }
    ...
    case 2:
        ...
        if($tst<=0){
            list($pers['x'], $pers['y']) = explode('_', $pers['pos']);
            $grasssql=mysqli_query($GLOBALS['db_link'],"SELECT * FROM `nature_fish` WHERE `x`='".$pers['x']."' AND `y`='".$pers['y']."';");
            if(mysqli_num_rows($grasssql)<1){$error="<font class=proce>Тут рыбы нет, или уровень вашего навыка недостаточен для ловли рыбы в этом месте.</font>";}
            else{
                    ...
                    while ($row = mysqli_fetch_assoc($grasssql)){	
                        ...
                        if(count($grnadd)>0){
                            ...
                            if(rand(1,9)==1){
                                ...
                                if($upcoeff>250){$upcoeff=250;}
                                if($upcoeff<25){$upcoeff=25;}
                        ...
                        }else{$error .= "<br><font class=proce>Тут рыбы нет, или вы использовали неправильную приманку.</font>";}
                    }
                }
            }	
            $captcha="00000";
