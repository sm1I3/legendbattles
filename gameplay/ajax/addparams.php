...
if($hash==$hashBD){
	...
	if($str){
		if($pers['baks']>=$_GET['sum'] and $_GET['sum']>0 and is_numeric($_GET['sum'])){
			...
			}else{echo 'Ошибка (id: ddparams-3).&nbsp;Попробуйте еще раз.&nbsp;Если повторится:&nbsp; напишите персонажу Администрация.';}
		}else{echo 'Недостаточно денег.<br>(Оплата производится в $).<br>Обменяйте имеющиеся DLR на $.&nbsp;Попробуйте еще раз';}
	}else{echo 'Ошибка!<br>Попробуйте еще раз.';}
}else{
