...
switch(intval($_GET['act'])){
	case 1:
		if($pers['id']){
			...
			else{$error='Ќавигатор недоступен в этой локации!';}
			$captcha="00000";
			header("Content-type: text/html; charset=windows-1251");
...
function findpath($x,$y,$xn,$yn){
		...
		$end['c'] = $xn."_".$yn;
		$closedset = "";		
		$start['f'] = way($start['c'],$end['c']); // Ёвристическа€ оценка рассто€ние до цели. h(x)
		$openset [] = $start['c'];
		$fset[]=$start['f'];
		$err='';
		$b=0;
	    while($openset){
			...
			if($low == $end['c']){
				$err = recpath($start,$kletka[$openset[$lowi]],$kletka); //заполн€ем карту path_map
				break;
			}
			unset($openset[$lowi]);
			unset($fset[$lowi]);
...
// ƒобавл€ем в карту все вершины от finish_node до start_node.
	$path_map = '';
    $current['c'] = $goal['c']; // поиск начинаетс€ от финиша
	while ($current['c']!=''){		
        $path_map .= $current['c']."|"; // ƒобавить вершину в карту
