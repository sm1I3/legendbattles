...
if($pers['login']=='Администрация'  or $pers['sign']==$sk){for($i=0;$i<count($trvtimer);$i++){$trvtimer[$i]=5;}$navi=1;}
$pers['les']+=$pt[60];
//////////////////////
...
case 1:
	...
	if(mysqli_num_rows($grasssql)<1 and $serp['id']){$error="Вы осмотрелись вокруг, но ничего не нашли.";}
	else{
		$error="";
		$grassrow="";
		...
		if($allgrass==""){$error="Вы осмотрелись вокруг, но ничего не нашли.";}		
	}
	...
	if($tst<=0){
		...
		if(mysqli_num_rows($grasssql)<1 and $serp['id']){$error="Вы осмотрелись вокруг, но ничего не нашли.";}
			...
			if($allgrass==""){$error="Вы осмотрелись вокруг, но ничего не нашли.";}
				...
				if($dolg<=0){$error.="<br><font class=proce style='color:black';><b>".$serp['name']." </b>истратил всю долговечность, приобретите новую.</font>";}
