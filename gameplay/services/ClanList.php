...
if(!empty($_GET['sign'])){
	if($_GET['sign'] == 'watchers'){
		...
		$ClanQuery = mysqli_query($GLOBALS['db_link'],"SELECT * FROM `user` WHERE `clan_id` = '".preg_replace('/[^a-zA-Zà-ÿÀ-ß0-9]/','',$_GET['sign'])."' ORDER BY `level` DESC");
	}
}
if(!empty($_GET['gif'])){
	$ClanQuery = mysqli_query($GLOBALS['db_link'],"SELECT * FROM `user` WHERE `clan_gif` = '".preg_replace('/[^a-zA-Zà-ÿÀ-ß0-9.]/','',$_GET['gif'])."' ORDER BY `level` DESC");
}
