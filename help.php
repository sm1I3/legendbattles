...
$error = $error ?? varcheck($_POST['error']) ?? varcheck($_GET['error']) ?? '';
if($error==1){
...
$clear = $clear ?? varcheck($_POST['clear']) ?? varcheck($_GET['clear']) ?? '';
if($clear==1){
	session_start(  );
	$_SESSION = array(  );
...
$stats = $stats ?? varcheck($_POST['stats']) ?? varcheck($_GET['stats']) ?? '';
if($stats==1){
echo'
...
$money = $money ?? varcheck($_POST['money']) ?? varcheck($_GET['money']) ?? '';
if($money==1){
...
$buffs = $buffs ?? varcheck($_POST['buffs']) ?? varcheck($_GET['buffs']) ?? '';
if($buffs==1){
...
$fish = $fish ?? varcheck($_POST['fish']) ?? varcheck($_GET['fish']) ?? '';
if($fish==1){
...
$taimer = $taimer ?? varcheck($_POST['taimer']) ?? varcheck($_GET['taimer']) ?? '';
if($taimer==1){
