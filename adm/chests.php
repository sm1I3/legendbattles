...
$NewChests = mysql_query("SELECT * FROM `items` WHERE `chests`='1';");
while($row = mysql_fetch_array($NewChests)){
	if(mysql_num_rows(mysql_query("SELECT * FROM `chests` WHERE `cid`='".$row['id']."'")) == 0){
		mysql_query("INSERT INTO `chests` (`cid`, `name`) VALUES ('".$row['id']."', '".$row['name']."');");
	}
}
$chests = mysql_query("SELECT * FROM `chests`;");
echo '
...
';
while($present = mysql_fetch_array($chests)){
	if(mysql_num_rows(mysql_query("SELECT * FROM `items` WHERE `chests`='1' and `id`='".$present['cid']."'")) > 0){
		echo '<option value="'.$present['id'].'" '.(($_POST['present']==$present['id'])?'selected=selected':'').'>'.$present['name'].'</option>';
	}else{
		mysql_query("DELETE FROM `chests` WHERE `cid`='".$present['cid']."'");
	}
}
echo '
</select>
...
if($_POST['present']!='none' and !empty($_POST['present'])){
	if(!empty($_POST['delete'])){
		$items=mysql_fetch_array(mysql_query("SELECT * FROM `chests` WHERE `id`='".$_POST['present']."' LIMIT 1;"));
		$additem="";
		switch($_POST['delete_id']){
			case 'items':
				...
				mysql_query("UPDATE `chests` SET `items`='".$additem."'  WHERE `id`='".$_POST['present']."'  LIMIT 1;");				
			break;
		}
	}
	if($_POST['idit']){
	...
	if($_GET['add']==1){
	$present=mysql_fetch_array(mysql_query("SELECT * FROM `chests` WHERE `id`='".$_POST['present']."';"));
	echo'
