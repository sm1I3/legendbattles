...
while ($row = mysqli_fetch_assoc($NewChests)) {
...
if ($_POST['present'] != 'none' and !empty($_POST['present'])) {
    if (!empty($_POST['delete'])) {
        $items = mysqli_fetch_assoc(mysqli_query($GLOBALS['db_link'], "SELECT * FROM `chests` WHERE `id`='" . $_POST['present'] . "' LIMIT 1;"));
        $additem = "";
    ...
    if ($_GET['add'] == 1) {
        $present = mysqli_fetch_assoc(mysqli_query($GLOBALS['db_link'], "SELECT * FROM `chests` WHERE `id`='" . $_POST['present'] . "';"));
        echo '
    ...
    if ($err == 0) {
        ...
        while ($row = mysqli_fetch_assoc($it)) {
            ...
            echo ">" . $row['name'] . " [ " . $row['level'] . " ] (" . ($row['dd_price'] > 0 ? $row['dd_price'] . ' reput' : $row['price'] . ' LR') . ")</option>";
        }
        echo '
			<input type=hidden name=present value="' . $_POST['present'] . '">
