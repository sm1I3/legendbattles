...
if (isset($_POST['action']) && $_POST['action']=='save') {
    ...
    if ($zone != '') {
        ...
        $query = '
            ...
            values
                ('.intval($x1).', '.intval($y1).', '.intval($x2).', '.intval($y2).', 9)';
        mysqli_query($GLOBALS['db_link'], $query);
...
<script src="jscript/tree_map.js" language="javascript"></script>
...
<tr>
    ...
    <td valign="top">
        ...
        <input type="button" name="create" value="Create"
               onclick="if (edit_mode == 0) { alert('Войдите в режим редактирования.'); } else { mode = 0; el('world_map').style.cursor = 'crosshair'; }"/>
