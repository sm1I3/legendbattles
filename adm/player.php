...
$inf = player();
$login = varcheck($_POST['login']) ?? varcheck($_GET['login']) ?? '';
?>
        ...
        <div class="TabbedPanelsContentGroup">
            <div class="TabbedPanelsContent">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <form action="player.php" method="post"><?
                        $load = varcheck($_POST['load']) ?? varcheck($_GET['load']) ?? '';
                        $loginp = varcheck($_POST['loginp']) ?? varcheck($_GET['loginp']) ?? '';
                        if ($load) {
                            $i = 0;
                        ...
                        $save = varcheck($_POST['save']) ?? varcheck($_GET['save']) ?? '';
                        $idp = varcheck($_POST['idp']) ?? varcheck($_GET['idp']) ?? '';
                        if ($save) {
                            $i = 0;
            ...
            <div class="TabbedPanelsContent">
                <form action="player.php" method="post">
                    ...
                    <?
                    $molch = varcheck($_POST['molch']) ?? varcheck($_GET['molch']) ?? '';
                    if ($molch) {
                    ...
                    $nomolch = varcheck($_POST['nomolch']) ?? varcheck($_GET['nomolch']) ?? '';
                    if ($nomolch) {
                        $pl = mysqli_fetch_assoc(mysqli_query($GLOBALS['db_link'], "SELECT * FROM user WHERE login='$login' LIMIT 1;"));
                        if ($pl['login'] != '') {
            ...
            <div class="TabbedPanelsContent">
                <form action="player.php" method="post">
                    <span class="logintext">Логин:</span> <input name="login" type="text" class="LogintextBox"/> <span
                            class="logintext">Сумма:</span> <input name="NV" type="text" value="0"
                                                                   class="LogintextBox2"/>
                    <input name="gmoney" type="submit" value="   Дать NV  " class="lbut"/>
                    <?
                    $gmoney = varcheck($_POST['gmoney']) ?? varcheck($_GET['gmoney']) ?? '';
                    $NV = varcheck($_POST['NV']) ?? varcheck($_GET['NV']) ?? '';
                    if ($gmoney) {
                        if ($NV != 0 and $login != '') {
            ...
            <div class="TabbedPanelsContent">
                <form action="player.php" method="post">
                    ...
                    <?
                    $free = varcheck($_POST['free']) ?? varcheck($_GET['free']) ?? null;
                    if ($free and $login != '') {
            ...
            <div class="TabbedPanelsContent">
                <form action="player.php" method="post" name="obraz">
                    <span class="logintext">Логин:</span> <input name="login" type="text" class="LogintextBox"/>
                    ...
                    <?
                    $obr = varcheck($_POST['obr']) ?? varcheck($_GET['obr']) ?? '';
                    $gif = varcheck($_POST['gif']) ?? varcheck($_GET['gif']) ?? '';
                    if ($obr and $login != '') {
            ...
            <div class="TabbedPanelsContent">
                ...
                <?
                $obn = varcheck($_POST['obn']) ?? varcheck($_GET['obn']) ?? '';
                if ($obn) {
                    if ($login != '') {
            ...
            <div class="TabbedPanelsContent">
                <form action="player.php" method="post"><br>
                    <span class="logintext">Логин:</span> <input name="login" type="text" class="LogintextBox"/> <span
                            class="logintext">Время в днях: </span> <input name="time" type="text" value="0"
                                                                           class="LogintextBox2"/>
                    <span class="logintext">Причина: </span> <input name="prich" type="text" maxlength="50"
                                                                    class="LogintextBox"/>
                    <input name="prison" type="submit" value="   В тюрьму  " class="lbut"/>
                    <input name="noprison" type="submit" value="   Выпустить  " class="lbut"/></form>
                <br><br>
                <?
                $prison = varcheck($_POST['prison']) ?? varcheck($_GET['prison']) ?? '';
                $prich = varcheck($_POST['prich']) ?? varcheck($_GET['prich']) ?? '';
                if ($prison) {
                    $pl = mysqli_fetch_assoc(mysqli_query($GLOBALS['db_link'], "SELECT * FROM user WHERE login='$login' LIMIT 1;"));
                ...
                $noprison = varcheck($_POST['noprison']) ?? varcheck($_GET['noprison']) ?? '';
            ...
            <div class="TabbedPanelsContent">
                <form action="player.php" method="post"><br>
                    <span class="logintext">Логин:</span> <input name="login" type="text" class="LogintextBox"/> <span
                            class="logintext">Причина: </span> <input name="prich" type="text" maxlength="50"
                                                                      class="LogintextBox"/>
                ...
                <br><br>
                <?
                $block = varcheck($_POST['block']) ?? varcheck($_GET['block']) ?? '';
                $prich = varcheck($_POST['prich']) ?? varcheck($_GET['prich']) ?? '';
                if ($block) {
                    $pl = mysqli_fetch_assoc(mysqli_query($GLOBALS['db_link'], "SELECT * FROM user WHERE login='$login' LIMIT 1;"));
                    if ($pl['login'] != '') {
                ...
                $unblock = varcheck($_POST['unblock']) ?? varcheck($_GET['unblock']) ?? '';
            ...
            <div class="TabbedPanelsContent">
                <form action="player.php" method="post"><br>
                    <span class="logintext">Сообщение:</span> <input name="message" type="text" length=1000
                                                                     class="LogintextBox"/>
...
//    chmsg($ms,'');
            ...
            <div class="TabbedPanelsContent">
                ...
                <br><br>
                <?
                $statsp = varcheck($_POST['statsp']) ?? varcheck($_GET['statsp']) ?? '';
                if ($statsp) {
            ...
            <div class="TabbedPanelsContent">
                ...
                <br><br>
                <?
                $koeffart = varcheck($_POST['koeffart']) ?? varcheck($_GET['koeffart']) ?? '';
                if ($koeffart) {/*
            ...
            function obnul($login)
            {
                ...
                switch ($pl['level']) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        $a = array(1 => 24, 2, 13, 24);
                        break;
                    case 4:
                        $a = array(1 => 29, 3, 18, 28);
                        break;
                    case 5:
                        $a = array(1 => 34, 3, 23, 33);
                        break;
                    case 6:
                        $a = array(1 => 39, 3, 29, 39);
                        break;
                    case 7:
                        $a = array(1 => 49, 3, 36, 45);
                        break;
                    case 8:
                        $a = array(1 => 56, 4, 44, 52);
                        break;
                    case 9:
                        $a = array(1 => 71, 4, 53, 60);
                        break;
                    case 10:
                        $a = array(1 => 78, 5, 65, 70);
                        break;
                    case 11:
                        $a = array(1 => 90, 5, 70, 75);
                        break;
                    case 12:
                        $a = array(1 => 100, 6, 85, 85);
                        break;
                    case 13:
                        $a = array(1 => 112, 7, 111, 105);
                        break;
                    case 14:
                        $a = array(1 => 127, 7, 126, 120);
                        break;
                    case 15:
                        $a = array(1 => 139, 8, 138, 135);
                        break;
                    case 16:
                        $a = array(1 => 154, 9, 153, 155);
                        break;
                    case 17:
                        $a = array(1 => 169, 10, 168, 175);
                        break;
                    default:
                        $a = array(1 => 0, 0, 0, 0);
                }
